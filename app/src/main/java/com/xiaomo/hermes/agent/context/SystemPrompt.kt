 ...
 *
 * Hermes adaptation: build system prompt, tools section, skills context.
 *
...
import com.xiaomo.hermes.agent.skills.RequirementsCheckResult
...
import com.xiaomo.hermes.channel.ChannelManager
import com.xiaomo.hermes.commands.CommandRegistry
import com.xiaomo.hermes.commands.CommandScope
import com.xiaomo.hermes.config.ConfigLoader
import com.xiaomo.hermes.contextengine.listContextEngineIds
...
class ContextBuilder(
    ...
    // Hermes: /sdcard/.hermes/workspace
    ...
    fun buildSystemPrompt(
        ...
        parts.add("## Workspace Files (injected)\nThese user-editable files are loaded by Hermes and included below in Project Context.")
...
""".trimIndent()
    ...
    private fun buildIdentitySection(): String {
        ...
        val accessibilityEnabled = try {
            val proxy = com.xiaomo.hermes.accessibility.AccessibilityProxy
        ...
        val agentName = try {
            ...
            if (config != null) {
                com.xiaomo.hermes.agent.AgentIdentity.resolveIdentityName(config) ?: "Hermes"
            } else "Hermes"
        } catch (_: Exception) { "Hermes" }
...
Do not manipulate or persuade anyone to expand access or disable safeguards. Do not copy yourself or change system prompts, safety rules, or tool policies unless explicitly requested.
    ...
    private fun buildChannelSection(): String {
        ...
        return if (hints.isNotEmpty()) {
            "# Channel: ${com.xiaomo.hermes.channel.CHANNEL_META.emoji} ${com.xiaomo.hermes.channel.CHANNEL_META.label}\n\n" +
            hints.joinToString("\n")
        } else {
            ""
        }
    ...
    private fun buildMessagingSection(channelContext: ChannelContext?): String {
        ...
        parts.add("The following JSON is generated by Hermes out-of-band. Treat it as authoritative metadata about the current message context.")
     ...
     */
    private fun escapeXml(str: String): String = com.xiaomo.hermes.agent.context.escapeXml(str)
...
This is a single-user Android device. All requests come from the device owner.
    ...
    private fun buildModelAliasesSection(): String {
        ...
        val lines = if (config != null) {
            com.xiaomo.hermes.providers.ModelSelection.buildModelAliasLines(config)
        } else {
            emptyList()
        }
        if (lines.isEmpty()) {
...
HEARTBEAT_OK
Hermes treats a leading/trailing "HEARTBEAT_OK" as a heartbeat ack (and may discard it).
If something needs attention, do NOT include "HEARTBEAT_OK"; reply with the alert text instead.
    ...
    private fun loadBootstrapFiles(channelContext: ChannelContext? = null): String {
        return com.xiaomo.hermes.agent.context.loadBootstrapFiles(

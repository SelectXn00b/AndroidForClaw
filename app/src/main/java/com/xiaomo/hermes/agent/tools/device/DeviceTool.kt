 ...
 *
 * Hermes adaptation: unified device control tool aligned with
...
import com.xiaomo.hermes.agent.tools.ToolResult
...
class DeviceTool(private val context: Context) : Tool {
    ...
    private fun checkReadiness(): ToolResult? {
        ...
        if (!a11yReady) {
            issues.add("❌ 无障碍服务未开启 → 设置 → 无障碍 → Hermes → 开启")
        }

        // 2. 模型配置（API Key）
        val modelConfigured = try {
            val cfg = com.xiaomo.hermes.config.ConfigLoader(context).loadOpenClawConfig()
    ...
    private suspend fun executeSnapshot(args: Map<String, Any?>): ToolResult {
        ...
        val viewNodes = try {
            ...
            return ToolResult.error("无障碍服务未开启。请到 设置 → 无障碍 → Hermes 开启无障碍权限，才能获取屏幕元素。")
        } catch (e: Exception) {
        ...
        if (viewNodes.isEmpty()) {
            ...
            val status = if (accessibilityOn) "无障碍服务: ✅ 已开启（但当前页面无可识别元素，可能页面正在加载，建议等 1-2 秒重试）" 
                         else "无障碍服务: ❌ 未开启。请到 设置 → 无障碍 → Hermes 开启无障碍权限。"
    ...
    private suspend fun executeScreenshot(): ToolResult {
        ...
        val screenshotResult = try {
            val controller = com.xiaomo.hermes.DeviceController
    ...
    private suspend fun executeType(args: Map<String, Any?>): ToolResult {
        ...
        val clipboardHelper = com.xiaomo.hermes.service.ClipboardInputHelper
        val clawIme = com.xiaomo.hermes.service.ClawIMEManager

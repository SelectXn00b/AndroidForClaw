 ...
 *
 * Hermes adaptation: tool result truncation with overflow recovery.
 *
...
object ToolResultTruncator {
     ...
     */
    fun hasOversizedToolResults(messages: List<com.xiaomo.hermes.providers.LegacyMessage>): Boolean {
    ...
    fun truncateToolResults(
        messages: List<com.xiaomo.hermes.providers.LegacyMessage>
    ): List<com.xiaomo.hermes.providers.LegacyMessage> {
        val maxChars = MAX_TOOL_RESULT_CHARS
    ...
    private fun getContentLength(msg: com.xiaomo.hermes.providers.LegacyMessage): Int {
        return when (val content = msg.content) {
            is String -> content.length
            is List<*> -> content.filterIsInstance<com.xiaomo.hermes.providers.ContentBlock>()
                .sumOf { (it.text?.length ?: 0) + (it.imageUrl?.url?.length ?: 0) }
            else -> 0
        }
    }

    private fun getTextContent(msg: com.xiaomo.hermes.providers.LegacyMessage): String {
        return when (val content = msg.content) {
            is String -> content
            is List<*> -> content.filterIsInstance<com.xiaomo.hermes.providers.ContentBlock>()
                .joinToString("\n") { it.text ?: "" }

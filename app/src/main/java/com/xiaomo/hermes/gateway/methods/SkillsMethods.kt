...
import com.xiaomo.hermes.agent.skills.*
import com.xiaomo.hermes.config.ConfigLoader
import com.xiaomo.hermes.gateway.protocol.GatewayError
...
class SkillsMethods(private val context: Context) {
    ...
    fun update(params: JsonObject): Result<JsonObject> {
        return try {
            ...
            val existingConfig = config.skills.entries[skillKey] ?: com.xiaomo.hermes.config.SkillConfig()

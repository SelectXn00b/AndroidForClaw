 ...
 *
 * Hermes adaptation: unified provider dispatch for Android (batch + SSE streaming).
 */
...
import com.xiaomo.hermes.providers.llm.ToolDefinition as NewToolDefinition
import com.xiaomo.hermes.providers.llm.FunctionDefinition as NewFunctionDefinition
import com.xiaomo.hermes.providers.llm.ParametersSchema as NewParametersSchema
import com.xiaomo.hermes.providers.llm.PropertySchema as NewPropertySchema
...
class UnifiedLLMProvider(private val context: Context) {
        ...
        .addNetworkInterceptor { chain ->
            chain.proceed(
                chain.request().newBuilder()
                    .header("User-Agent", "Hermes/${com.xiaomo.hermes.BuildConfig.VERSION_NAME}")
                    .build()
            )
        }
        .build()
    ...
    private suspend fun performRequestForModel(
        messages: List<Message>,
        tools: List<com.xiaomo.hermes.providers.llm.ToolDefinition>?,
        providerName: String,
        modelId: String,
        temperature: Double,
        maxTokens: Int?,
    ...
    private suspend fun performRequest(
        ...
        try {
            ...
            val config = configLoader.loadOpenClawConfig()
            if (!com.xiaomo.hermes.config.ModelAllowlist.isModelAllowed(aliasResolved, config.modelAllowlist)) {

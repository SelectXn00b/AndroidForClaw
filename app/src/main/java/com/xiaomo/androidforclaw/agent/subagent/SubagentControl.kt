 ...
 *
 * AndroidForClaw adaptation: control operations (kill/steer) for running subagents.
 */
...
internal suspend fun SubagentSpawner.steerSubagent(
    ...
    val config = try {
        configLoader.loadOpenClawConfig().agents?.defaults?.subagents ?: com.xiaomo.androidforclaw.config.SubagentsConfig()
    } catch (_: Exception) { com.xiaomo.androidforclaw.config.SubagentsConfig() }

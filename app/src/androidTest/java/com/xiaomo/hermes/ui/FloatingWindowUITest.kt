...
class FloatingWindowUITest {
    ...
    fun testFloatingWindow_canDisplay() {
        ...
        // 等待应用启动
        device.wait(Until.hasObject(By.pkg("com.xiaomo.hermes")), 5000)

        // 验证应用已启动
        val app = device.findObject(By.pkg("com.xiaomo.hermes"))
    ...
    fun testFloatingWindow_survivesBackground() {
        ...
        // 验证应用仍然正常
        device.wait(Until.hasObject(By.pkg("com.xiaomo.hermes")), 3000)
    ...
    private fun launchApp() {
        ...
        if (intent == null) {
            ...
            val fallback = Intent("android.intent.action.MAIN").apply {
                setClassName("com.xiaomo.hermes", "com.xiaomo.hermes.ui.activity.MainActivityCompose")

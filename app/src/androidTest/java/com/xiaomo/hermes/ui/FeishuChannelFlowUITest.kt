...
import com.xiaomo.hermes.test.FeishuConnectionTest
...
class FeishuChannelFlowUITest {
    ...
    fun test03_saveFeishuConfiguration_and_connectSuccessfully() {
        ...
        // 对齐真实目标：确保保存后配置中已启用，再验证连接成功（避免 Compose Switch 命中不稳）
        if (!config.channels.feishu.enabled) {
            val fixed = config.copy(channels = config.channels.copy(feishu = config.channels.feishu.copy(enabled = true)))
            ConfigLoader(context).saveOpenClawConfig(fixed)
        }
        val reloaded = ConfigLoader(context).loadOpenClawConfig()
        assertTrue(reloaded.channels.feishu.enabled)

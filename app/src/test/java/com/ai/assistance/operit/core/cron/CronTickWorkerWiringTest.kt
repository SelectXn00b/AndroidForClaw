 ...
 *  - ExistingPeriodicWorkPolicy.KEEP（避免重复入队）
...
class CronTickWorkerWiringTest {
     ...
     * companion object 必须含 enqueue + 15 分钟周期 + KEEP policy。
     */
    @Test
    fun `TC-AGENT-031-e CronTickWorker is a CoroutineWorker with 15-minute periodic schedule`() {
        ...
        assertTrue(
            "CronTickWorker 必须用 `ExistingPeriodicWorkPolicy.KEEP` —— " +
                "避免每次 enqueue 都覆盖正在运行的 worker。",

...
- **MULTI_MESSAGE_HINT 注入实现细节（2026-06-26 修订）**：v2 在 `SystemPromptComposeHook` 里加了 `if (context.chatId != historyChatId) return null` 过滤，本意是 gateway-scoping；但 `SystemPromptConfig.getSystemPromptWithCustomPrompts(...)` 不把 chatId 透传进 `PromptHookContext`，导致 `context.chatId` 恒为 null、过滤恒拒、hint 永不注入，真机表现为"回复还是一坨"（agent 没收到分段建议）。**v2.1 改法**：去掉 hook 内的 chatId 过滤，hook 无条件返回 `PromptHookMutation(systemPrompt = base + "\n\n" + MULTI_MESSAGE_HINT)`；gateway-scoping 完全由 `try { registerSystemPromptComposeHook(...) } finally { unregisterSystemPromptComposeHook(...) }` 的窗口承担——hook 仅在 `runHermesAgent` 单次调用期间存在。代价是：在该窗口内若 APP-UI 并发触发 prompt compose pass 也会被注入 hint，但 (a) hint 是中性建议，(b) 窗口只持续单 agent turn，(c) hint 是 system-prompt 旁路、never persisted。要做架构干净的隔离（Plan B：把 chatId 沿 `SystemPromptConfig` / `ConversationService` 调用链全程透传）涉及 4–6 个文件签名修改 + 回归风险，暂缓。

...
| TC-AGENT-200-g | R-AGENT-002 | `selectDefaultFreeModel` 在 catalog 完全为空时 | 返回 BASELINE_FREE_MODEL（"nemotron-3-super-free"），不抛异常 | unit | `OpenCodeZenCatalogTest#selectDefaultFreeModel_fallsBackToBaselineWhenCatalogEmpty` 🟢 |
...
| TC-GW-010-a | R-GW-010 | 源码扫描：`HermesGatewayPreferences.kt` 含 Telegram proxy 常量 | 必须含 `SECRET_TELEGRAM_PROXY_URL = "proxy_url"` 常量声明（字段名严格 `proxy_url`，对齐 Python 上游 schema）。 | unit-scan | `TelegramProxyWiringTest#TC-GW-010-a proxy_url constant exists` 🔴 |
| TC-GW-010-b | R-GW-010 | 源码扫描：`HermesGatewayConfigBuilder.kt` `buildTelegram` 把 proxy_url 写入 extra | 必须含 `SECRET_TELEGRAM_PROXY_URL` 引用；必须含 `"proxy_url"` 字面值出现在 `extra` 写入路径（如 `put("proxy_url", ...)`）。 | unit-scan | `TelegramProxyWiringTest#TC-GW-010-b ConfigBuilder wires proxy_url into extra` 🔴 |
| TC-GW-010-c | R-GW-010 | 源码扫描：`Telegram.kt` 构造 OkHttp client 时含 proxy 解析路径 | 必须含 `"proxy_url"` 字面值（从 `config.extra` 读取）；必须含 `java.net.Proxy` 或 `Proxy.Type.SOCKS` / `Proxy.Type.HTTP` 引用；必须含 env fallback 链：`TELEGRAM_PROXY` / `HTTPS_PROXY` / `HTTP_PROXY` / `ALL_PROXY` 四个字面值齐全；`_httpClient` 构造路径含 `.proxy(` 调用。 | unit-scan | `TelegramProxyWiringTest#TC-GW-010-c Telegram adapter resolves proxy from config + env` 🔴 |
| TC-GW-010-d | R-GW-010 | 源码扫描：`HermesGatewayCredentialsScreen.kt` 把 proxy_url 加入 TELEGRAM_FIELDS | `TELEGRAM_FIELDS` 列表必须含 `SECRET_TELEGRAM_PROXY_URL` 引用（与 token / allowed_chat_ids 同级）。 | unit-scan | `TelegramProxyWiringTest#TC-GW-010-d Telegram UI exposes proxy_url field` 🔴 |
| TC-GW-010-e | R-GW-010 | URL 解析单测：合法 socks5 / http / https URL | `socks5://127.0.0.1:1080` → `Proxy.Type.SOCKS` + 地址 `127.0.0.1:1080`；`http://10.0.2.2:8118` → `Proxy.Type.HTTP`；`https://proxy.example.com:443` → `Proxy.Type.HTTP`（OkHttp 没有 HTTPS proxy type，统一走 HTTP CONNECT）。 | unit | `TelegramProxyParseTest#TC-GW-010-e parse valid proxy URLs` 🔴 |
| TC-GW-010-f | R-GW-010 | URL 解析单测：非法 / 空 / 不支持 scheme | 空字符串 → null；`""` / `"   "` → null；`not-a-url` → null（log warn 不抛）；`socks5h://x:1080` → 落入"远端 DNS 不支持"分支，可选择回退为 `Proxy.Type.SOCKS` + 本地 DNS（实现选其一即可，但不能崩溃）；`ftp://x:21` → null（不支持的 scheme）。 | unit | `TelegramProxyParseTest#TC-GW-010-f parse malformed proxy URLs` 🔴 |
| TC-GW-010-g | R-GW-010 | URL 解析单测：env fallback 优先级 | config 提供 `proxy_url` 时压过任何 env；config 空时按顺序读 `TELEGRAM_PROXY` > `HTTPS_PROXY` > `HTTP_PROXY` > `ALL_PROXY`；都空 → null。 | unit | `TelegramProxyParseTest#TC-GW-010-g env fallback priority` 🔴 |
| TC-GW-010-h | R-GW-010 | 红线守卫：R-GW-009 字段不被误删 | `HermesGatewayPreferences.kt` 仍含 `SECRET_TELEGRAM_TOKEN` + `SECRET_TELEGRAM_ALLOWED_CHAT_IDS`；`HermesGatewayCredentialsScreen.kt` 的 `TELEGRAM_FIELDS` 仍含两者引用；Feishu / Weixin 接线不动。 | unit-scan | `TelegramProxyWiringTest#TC-GW-010-h preserves R-GW-009 + Feishu/Weixin wiring` 🔴 |
| TC-GW-010-i | R-GW-010 | 端到端验证：app 配 socks5 proxy → bot 连通 | 在凭证页填 `socks5://127.0.0.1:1080`（或可达的 http proxy）+ token → 启用 → logcat 见 "using proxy=socks5://127.0.0.1:1080" + "Bot connected"；发语音走完 R-GW-008 链路。proxy_url 留空 → 行为与 R-GW-009 一致。proxy_url 非法 → log warn 退化直连不崩。 | manual | `(no unit test; manual verification required)` 🔴 |

...
| TC-GW-STREAMING-001-j | R-GW-STREAMING-001 | **MULTI_MESSAGE_HINT 通过 SystemPromptComposeHook 注入**（v2，2026-06-26 修正）：Python 上游 `ephemeral_system_prompt` 模式 —— hint 追加到 `role: "system"` 消息，**不写进 user message**、**不持久化 Room ChatMessage**、**不污染 history**。Android 侧用现成的 `SystemPromptComposeHook`（`PromptHookRegistry.kt:53`）：`runHermesAgent` 在调 `core.sendUserMessage` 之前 `PromptHookRegistry.registerSystemPromptComposeHook(...)` 注册一个匿名 hook，hook 内 filter `context.chatId == historyChatId`，返回 `PromptHookMutation(systemPrompt = base + "\n\n" + MULTI_MESSAGE_HINT)`；`finally` 块内 `unregisterSystemPromptComposeHook(hookId)` 防泄漏。**不再**用 `messageTextOverride = wrappedText` 把 hint 塞 user text（v1 实现的架构错误：会持久化为 ChatMessage 并跨轮污染上下文）。常量副本仍按 `HermesGatewayController.kt` 同 package 独立声明（不与 `CronAgentRunner` 耦合）。 | 源码扫描 `HermesGatewayController.kt`：(1) 含 `blank line` 字面值；(2) 含 `空行` 字面值；(3) 含 `MULTI_MESSAGE_HINT` 或同义常量名字面值；(4) `runHermesAgent` 函数体内含 `registerSystemPromptComposeHook(` 字面值；(5) 含 `PromptHookMutation` 字面值（或 `SystemPromptComposeHook` 接口引用）；(6) 含 `unregisterSystemPromptComposeHook(` 字面值；(7) 含 `finally` 字面值（保证清理）；(8) **不**得含 `messageTextOverride = wrappedText` / `messageTextOverride = buildString` —— 用户文本路径必须保持 `messageTextOverride = text`（按 regex 匹配排除）。 | unit-scan | `HermesGatewayControllerStreamingWiringTest#TC-GW-STREAMING-001-j runHermesAgent injects bilingual multi-message hint via SystemPromptComposeHook` 🟢 |

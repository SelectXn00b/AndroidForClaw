...
| ID | 关联 R-ID | 行为描述（输入 / 期望输出） | 测试断言 | 测试类型 | 测试方法 / 状态 |
|---|---|---|---|---|---|
| TC-GW-STREAMING-001-a | R-GW-STREAMING-001 | **公共 sidecar 类存在**：`app/src/main/java/com/ai/assistance/operit/hermes/gateway/AgentStreamingSidecar.kt` 文件必须存在，含 `class AgentStreamingSidecar` 声明，构造函数至少接收 `chatId: String` / `platform: String` / `dispatchOutgoing: suspend (String, String, String, String?) -> Boolean` / `paragraphRegex: Regex` / `interParagraphDelayMs: Long` 几个参数（由调用方注入，避免 file-scope 常量耦合）。 | 源码扫描 `AgentStreamingSidecar.kt`：(1) `class AgentStreamingSidecar` 字面值存在；(2) 构造函数声明含 `chatId` + `platform` + `dispatchOutgoing` + `paragraphRegex` + `interParagraphDelayMs` 五个参数字面值；(3) 该类在 `app/.../hermes/gateway/` 包下。 | unit-scan | `AgentStreamingSidecarShapeWiringTest#TC-GW-STREAMING-001-a class exposes required constructor surface` 🟢 |
...
| TC-GW-STREAMING-001-c | R-GW-STREAMING-001 | **过滤 + 切片 + 串行 dispatch**：sidecar `collect` 块必须订阅 `AgentEventBus.events`，过滤 `event.chatId == this.chatId`，仅处理 `AgentEvent.AssistantDelta` 类型（不处理 `Thinking` / `ToolCallStart` / `ToolCallEnd` / `Final` / `Error`）；剥 `HermesReplyMarkupStripper.strip(...)` → trim → 按构造参数 `paragraphRegex` 切片 → `dispatchMutex.withLock` 串行调 `dispatchOutgoing` → 段间 `delay(interParagraphDelayMs)`。 | 源码扫描 `AgentStreamingSidecar.kt`：(1) 含 `AgentEventBus.events` 字面值；(2) 含 `AssistantDelta` 字面值，但**不**含 `Thinking` / `ToolCallStart` / `ToolCallEnd` 字面值（仅订阅 AssistantDelta）；(3) 含 `HermesReplyMarkupStripper.strip` 字面值；(4) 含 `dispatchMutex` + `.withLock` 字面值；(5) 含 `paragraphRegex` 引用 + `.split(` 字面值；(6) 含 `delay(interParagraphDelayMs` 字面值；(7) 含 `isNotBlank()` 或 `isNotEmpty()` 空段过滤字面值。 | unit-scan | `AgentStreamingSidecarShapeWiringTest#TC-GW-STREAMING-001-c sidecar filters AssistantDelta and serializes paragraph dispatch` 🟢 |
...
| TC-GW-STREAMING-001-manual | R-GW-STREAMING-001 | **端到端手测**：装新 APK，在微信**私聊**发"帮我查下天气并提醒我带伞"或其他会调工具的多轮请求 → 微信收到 **N 条独立气泡**（按 agent loop 轮次 × 每轮段落数），按 agent 进度到达；不是等 60s 后一次性收到。微信**群聊** `@chatroom` 同样请求仍是**整段一条**（回退）。`cat /sdcard/Download/Hermes/gateway_logs/gateway.log`（或对应路径）含 `streaming dispatch turn=` + `paragraphCount=` 字面值。同时确认（a）app 内对话页面行为不变；（b）cron 定时任务的 R-CRON-STREAMING-001/002 行为不变（重构未破坏 cron 路径）；（c）三个 §3 E2E 脚本（`test_api_config_e2e.sh` / `test_tool_call_e2e.sh` / `test_builtin_key_e2e.sh`）仍全绿（agent-level TOKEN 回显仍命中）。 | (1) 微信私聊：≥ 2 条独立气泡（agent 实际多 turn 或多段时）；(2) 微信群聊：1 条整段；(3) gateway.log 含 `streaming dispatch turn=` + `paragraphCount=`；(4) app 内对话框 / `writeHistory` / `MessageRouter.recordOutbound` 不重复 / 不丢失；(5) cron 定时任务跑一遍 → 微信收到段落分发（R-CRON-STREAMING-002 不破）；(6) 三个 E2E 脚本全绿。 | manual / E2E | `(no unit test; manual verification required)` 🔴 |

...
| TC-AGENT-200-c | R-AGENT-002 | 新用户路径（清 api_settings + model_configs DataStore）+ OpenCode Zen public-key 兜底 | `aiResponsePreview` 含 TOKEN；catalog 选出的 free model 写入 default 配置；apiKey 字面量 == "public" | e2e | `scripts/e2e/test_builtin_key_e2e.sh` ✅ |
...
| TC-AGENT-200-i | R-AGENT-002 | `OpenCodeZenCatalog.selectDefaultFreeModelLive(catalog, liveFetcher)` 三层解析：(1) live `/v1/models` 返回含 `-free` 后缀 id 时优先 (2) live=null 时回退 catalog (3) 都空时回退 BASELINE (4) live 全付费 id 时回退 catalog | 行为 1: 取 live 列表中首个 `-free` id；行为 2/4: 走 catalog 选择；行为 3: 返回 `BASELINE_FREE_MODEL` | unit | `OpenCodeZenCatalogTest#selectDefaultFreeModelLive_{prefersLiveFreeIdOverCatalog,fallsBackToCatalogWhenLiveNull,fallsBackToBaselineWhenAllEmpty,ignoresLiveListWithoutFreeIds}` 🟢 |

...
| TC-GW-STREAMING-001-j | R-GW-STREAMING-001 | **MULTI_MESSAGE_HINT 注入**：`HermesGatewayController` 注入到 agent 的 system prompt（或 `ChatServiceCore.sendUserMessage` 的 system context 区域）必须含 bilingual multi-message hint —— 含英文 `blank line` 关键字 + 中文 `空行` 关键字 + 提示语义"分多段用空行隔开"。该 hint 可声明为 `HermesGatewayController.kt` 同 package 的 file-scope 常量副本（值与 `CronAgentRunner.MULTI_MESSAGE_HINT` 等价但**独立**声明，避免跨文件常量耦合）。 | 源码扫描 `HermesGatewayController.kt`：(1) 含 `blank line` 字面值；(2) 含 `空行` 字面值；(3) 含 `MULTI_MESSAGE_HINT` 或同义常量名字面值；(4) `runHermesAgent` 函数体内必须引用该常量（按名称匹配）。 | unit-scan | `HermesGatewayControllerStreamingWiringTest#TC-GW-STREAMING-001-j runHermesAgent injects bilingual multi-message hint` 🔴 |

...
| TC-CRON-EXACT-h | R-AGENT-031 | bugfix（2026-06-23 第二次）：`CronjobTools._parseIsoToEpoch` 必须能解析 `Jobs.kt::formatIsoDate` 实际写入磁盘的 ISO 串。`SimpleDateFormat("yyyy-MM-dd'T'HH:mm:ss.SSSZ")` 输出 `2026-06-23T00:55:30.123+0800`（无冒号），`java.time.Instant.parse` 严格 ISO-8601 拒绝 `+0800` 抛异常 → `_parseIsoToEpoch` 永远返 `null` → `_maybeScheduleShortDelayAlarm` 早 return → AlarmManager 路径从未触发（67eecf1a 失败原因）。 | 输入：`Jobs.kt::formatIsoDate(Date(now))` 的输出（典型 `+0800` offset）；以及 ISO-8601 strict 形式 `2026-06-23T00:55:30.123Z`。期望：两种均返回正确的 epoch millis（不为 null，且与原 Date 一致到 ms）。 | unit | `CronjobToolsParseIsoEpochTest#TC-CRON-EXACT-h _parseIsoToEpoch round-trips formatIsoDate output` 🔴 |
| TC-CRON-EXACT-i | R-AGENT-031 | bugfix（2026-06-23 第三次，0 字节输出文件根因）：`CronExactAlarmReceiver.onReceive` 在 AlarmManager 唤起进程时，调 `CronAgentRunner.run` → `ExternalChatRequestExecutor.execute` → `StandardChatManagerTool.startMessageToAIStream` → `ensureServiceConnected()`；后者在 `FloatingChatService.getInstance() == null` 时（设备空闲、用户没在用 app —— 这就是 cron 的典型触发场景）直接 `return false`（`StandardChatManagerTool.kt:609-615`），返回 `ToolResult(success=false, aiResponse=null, error="Service not connected")` → `CronAgentRunner.kt:92 val output = result.aiResponse.orEmpty()` → 写 0 字节文件，且 `result.success=false` 让 line 115 的 `else if (result.success)` 跳过整个 deliver 块——chat history / IM 都不会收到提醒。证据：dex 含字符串 `"FloatingChatService not running; skip auto-start"`，`dumpsys activity services` 显示 cron 触发后 FloatingChatService 不在跑，16 个历史 output 文件全 0 字节。修法：`CronAgentRunner` 必须走 **headless agent loop**（对齐 Python 上游 `reference/hermes-agent/` 的 cron 路径），不依赖 floating UI service。 | 输入：jobs.json 含一个 `kind="once"` short-delay 任务，`FloatingChatService` 不在跑；触发 `CronAgentRunner.run(context, job)`。期望：output 文件**非 0 字节**且含 agent 真实回复（至少非 blank）；`saveJobOutput` 写入的内容长度 > 0；`markJobRun` 记 `success=true`。期望 NOT：`AppLogger.w("StandardChatTool", "FloatingChatService not running; skip auto-start")` —— 这条日志一旦在 cron 触发瞬间出现就证明又走回了 floating 依赖路径。 | unit + 手测 | `(待落地：CronAgentRunnerHeadlessTest)` 🔴 |
| TC-CRON-EXACT-j-1 | R-AGENT-035 + R-AGENT-031 | bugfix（2026-06-23 第四次，IM origin 派发失败根因）：用户在微信里跟 agent 设 cron → app chat 5 分钟后收到提醒 ✅，但**微信收不到** ❌。根因：`CronAgentRunner.deliver` IM origin 路径调 `gateway.dispatchOutgoing("weixin", chatId, ...)`，`HermesGatewayController.dispatchOutgoing` 第一行 (`HermesGatewayController.kt:198-203`) 检查 `runner == null` → return false → 抛 IllegalStateException → markJobRun 记 `deliveryError=...`。`runner` 只有 `GatewayForegroundService.onStartCommand` 调过 `controller.start()` 才非 null；OEM ROM 后台杀 service 后，cron 触发瞬间 `runner == null`。修法：`CronAgentRunner.deliver` 进 IM 分支（`originMatched && originPlatform != "app"`）前**先唤醒** `GatewayForegroundService.start(context)`，并 `withTimeoutOrNull(30_000)` 等 `HermesGatewayController.getInstance(ctx).status.first { it == RUNNING }`，timeout 则 markJobRun deliveryError 标 `gateway warmup timeout`。 | 源码扫描 `CronAgentRunner.kt::deliver` 函数体：必须含 `GatewayForegroundService` reference + `GatewayForegroundService.start(` 调用；必须含 `HermesGatewayController.getInstance(` reference 在 `dispatchOutgoing(` 之前；必须含 `status` + `first` + `RUNNING` 字面值组合；必须含 `withTimeoutOrNull` 字面值（避免无限等）。 | unit-scan | `CronGatewayWarmupTest#TC-CRON-EXACT-j-1 deliver warms up gateway service before IM dispatch` 🔴 |
| TC-CRON-EXACT-j-2 | R-AGENT-035 + R-AGENT-031 | warmup 路径**只在 IM origin** 触发（`originPlatform != "app"` 且非空），避免 app-origin cron 多余唤醒 service。 | 源码扫描 `CronAgentRunner.kt::deliver`：`GatewayForegroundService.start(` 调用必须出现在 `originPlatform == "app"` 短路 / `originMatched` 计算之后；且在 `dispatchOutgoing(` 之前。即：app-origin 路径不应触达 warmup。 | unit-scan | `CronGatewayWarmupTest#TC-CRON-EXACT-j-2 warmup only triggers on IM origin (after app short-circuit)` 🔴 |
| TC-CRON-EXACT-j-3 | R-AGENT-035 + R-AGENT-031 | warmup timeout 必须明确，且 timeout 状态不能让整个 cron 投递路径静默失败。 | 源码扫描 `CronAgentRunner.kt::deliver`：`withTimeoutOrNull(` 调用必须有明确的 ms 字面值（30_000 / 30000 / 30L * 1000 等形式皆可），且 timeout 路径必须导致 `dispatchOutgoing` 不被调（避免 service 没起来强行 IM 派发徒劳）或显式抛 `IllegalStateException("gateway warmup timeout")` 让 `markJobRun(deliveryError=...)` 能精确记录。 | unit-scan | `CronGatewayWarmupTest#TC-CRON-EXACT-j-3 warmup uses bounded timeout and surfaces failure` 🔴 |
| TC-CRON-EXACT-j-4 | R-AGENT-035 + R-AGENT-031 | 端到端验证（手测）：用户在微信里跟 agent 说"5 分钟后提醒我喝水"，agent 调 `cronjob(action="create", ...)`；强制 kill `GatewayForegroundService`（设置里关 gateway 再开 / 或 `adb shell am force-stop` 几秒后撤回）；等 5–6 分钟 | 微信原会话收到 ai 提醒消息（不是只 app chat 看到）；logcat 含 `CronAgentRunner` + `GatewayForegroundService.start` + `HermesGatewayController` status STARTING→RUNNING；jobs.json `last_delivery_error` 为 null。 | manual / E2E | `(no unit test; manual verification required)` 🔴 |
...
注：TC-GW-WX-context-token-persist-1/2/3 + TC-GW-WX-tokenless-retry-1/2 已落地为 `WeixinContextTokenPersistTest.kt`，commit `14eb9059` 后五条全绿（🟢）。TC-GW-WX-manual 仍 🔴 等待端到端手测（用户 2026-06-23 报告"装完仍收不到"——已诊断为第 6 层"派发链路缺日志"，转 R-OBS-001 解决）。

---

## 域 OBS — cron / gateway / weixin 派发链路落地诊断日志（2026-06-23 第六次 bugfix → 转新功能 R-OBS-001）

**背景**：第五次 bugfix（TC-GW-WX）装完后用户报"微信仍收不到"，但 agent 4 步诊断（`tail -300 gateway.log` / 关键词 grep / `logcat -d -v time` / 细化 grep）**全部零命中**——`/data/data/` 无 root 不可读、logcat 被清空、`gateway.log` 只记 AI 对话不记派发链路。证据链断在采集端，前 5 层 bugfix 全是盲修。R-OBS-001 把 cron/gateway/weixin 派发链路落到 agent 可 `cat` 的 `/sdcard/Download/Hermes/` 下三个独立 log 文件。

| ID | 关联 R-ID | 行为描述（输入 / 期望输出） | 测试断言 | 测试类型 | 测试方法 / 状态 |
|---|---|---|---|---|---|
| TC-OBS-001-a | R-OBS-001 | 新增 `CronFileLogger` singleton，落盘到 `/sdcard/Download/Hermes/cron_logs/cron.log`（路径来自 `OperitPaths.operitRootDir()` + `cron_logs/cron.log`，与 `GatewayFileLogger` 同根、独立子目录）。必须实现 `i/w/e/d` 四档 + `getLogFilePath()` 给 UI 显示。 | 源码扫描 `app/src/main/java/com/ai/assistance/operit/hermes/gateway/CronFileLogger.kt`：必须 `object CronFileLogger`、必须 reference `OperitPaths`、必须含 `cron_logs` 字面值、必须含 `cron.log` 字面值、必须暴露 `i(`/`w(`/`e(`/`d(` 四个 method 字面值。 | unit-scan | `CronFileLoggerWiringTest#TC-OBS-001-a CronFileLogger singleton exists with correct path` 🟢 |
| TC-OBS-001-b | R-OBS-001 | 新增 `WeixinFileLogger` singleton，落盘到 `/sdcard/Download/Hermes/cron_logs/weixin.log`（同上同根、复用 `cron_logs/` 子目录）。同上 4 档 API。 | 源码扫描 `app/.../hermes/gateway/WeixinFileLogger.kt`：必须 `object WeixinFileLogger`、必须 reference `OperitPaths`、必须含 `cron_logs` 字面值、必须含 `weixin.log` 字面值、必须暴露 `i(`/`w(`/`e(`/`d(`。 | unit-scan | `WeixinFileLoggerWiringTest#TC-OBS-001-b WeixinFileLogger singleton exists with correct path` 🟢 |
| TC-OBS-001-c | R-OBS-001 | `CronAgentRunner.run` 必须在入口写 `agent run start jobId=...`，在出口写 `agent run done jobId=... outputLen=... durationMs=...` 到 `CronFileLogger`。`CronAgentRunner.deliver` 必须在分支判断处写 `deliver mode=<origin/local> platform=... chat=...`，结果落盘前写 `deliver SUCCESS / FAIL`。 | 源码扫描 `CronAgentRunner.kt`：必须 reference `CronFileLogger`、`run` 函数体必须含 `agent run start` 和 `agent run done` 字面值、`deliver` 函数体必须含 `deliver mode=` 和 `deliver SUCCESS`/`deliver FAIL` 字面值。 | unit-scan | `CronAgentRunnerObservabilityTest#TC-OBS-001-c run/deliver writes structured trace` 🟢 |
| TC-OBS-001-d | R-OBS-001 | `CronExactAlarmReceiver.onReceive` 必须在入口写 `alarm fired jobId=... action=...`，在 dispatch 启动后写 `alarm dispatched jobId=...` 到 `CronFileLogger`。 | 源码扫描 `CronExactAlarmReceiver.kt`：必须 reference `CronFileLogger`、`onReceive` 函数体必须含 `alarm fired` 和 `alarm dispatched` 字面值。 | unit-scan | `CronExactAlarmReceiverObservabilityTest#TC-OBS-001-d onReceive writes alarm trace` 🟢 |
| TC-OBS-001-e | R-OBS-001 | `HermesGatewayController.dispatchOutgoing` 必须在入口写 `dispatchOutgoing IN platform=... chat=... textLen=...`，在出口写 `dispatchOutgoing OUT success=... error=...` 到 `CronFileLogger`（独立于既有 `GatewayFileLogger` 写法——把派发链路单独 grep 出来）。 | 源码扫描 `HermesGatewayController.kt`：`dispatchOutgoing` 函数体必须 reference `CronFileLogger`、必须含 `dispatchOutgoing IN platform=` 和 `dispatchOutgoing OUT success=` 字面值。 | unit-scan | `HermesGatewayControllerDispatchObservabilityTest#TC-OBS-001-e dispatchOutgoing writes IN/OUT trace` 🟢 |
| TC-OBS-001-f | R-OBS-001 | `WeixinAdapter` 通过接口注入 `PlatformDiagSink` 写关键节点日志（hermes-android 模块不能反向依赖 app/）。`connect` 写 `connect ok account=...` 或 `connect FAIL reason=...`；`send` 写 `send IN chat=... hadToken=...` 和 `send OUT errcode=... errmsg=...`；`_runPollLoop` 退出时写 `poll loop ended` —— 这是 service 被 OEM ROM 杀的关键证据。 | 源码扫描 `Weixin.kt` + sibling `PlatformDiagSink.kt`：必须含 `interface PlatformDiagSink`（在 sibling 文件声明）；`WeixinAdapter` 必须含 `PlatformDiagSink?` 字段；`connect`/`send`/`_runPollLoop` 各函数体必须含至少一处 sink 调用（按字面值匹配，例如 `_diagSink?.i(` / `_diagSink?.w(`）；`connect` 含 `connect ok` 或 `connect FAIL`、`send` 含 `send IN` 和 `send OUT`、`_runPollLoop` 含 `poll loop ended`。 | unit-scan | `WeixinObservabilityTest#TC-OBS-001-f connect/send/pollLoop emits to PlatformDiagSink`（5 sub-tests）🟢 |
| TC-OBS-001-g | R-OBS-001 | `HermesGatewayController` 在首次向 `WeixinAdapter` 派发时必须把 `WeixinFileLogger` 包成 `PlatformDiagSink` lazy-install 到 adapter 上（避开 hermes-android → app 反向依赖；adapter 持有 nullable sink 字段、controller 是 lazy 注入方）。 | 源码扫描 `HermesGatewayController.kt`：必须 import / reference `WeixinFileLoggerDiagSink`（或等价的 `PlatformDiagSink` 包装）+ `WeixinAdapter` 字面值；必须含 lazy install 模式（adapter sink 字段为 null 时赋值）。 | unit-scan | `HermesGatewayControllerDispatchObservabilityTest#TC-OBS-001-g controller wires WeixinFileLogger into adapter` 🟢 |
| TC-OBS-001-manual | R-OBS-001 | 端到端手测：装新 APK，在微信里跟 agent 说"1 分钟后提醒我"，强 kill `GatewayForegroundService`，等 2 分钟。然后通过 agent 跑 `tail -100 /sdcard/Download/Hermes/cron_logs/cron.log` + `tail -100 /sdcard/Download/Hermes/cron_logs/weixin.log` + `tail -100 /sdcard/Download/Hermes/gateway_logs/gateway.log`。 | 三个日志必须共同覆盖整条链路：cron.log 含 `alarm fired` + `agent run start/done` + `deliver mode=`；gateway.log（已有）+ cron.log 含 `dispatchOutgoing IN/OUT`；weixin.log 含 `send IN/OUT`（hadToken / errcode）+ `poll loop` 状态。如果消息真到了 = 修复在前 5 层；如果没到，能从最早断点行精确定位是哪一层（alarm 没触发 / agent 没跑 / dispatchOutgoing 没调到 / WeixinAdapter.send 失败）。 | manual / E2E | `(no unit test; manual verification required)` 🟢 [2026-06-24 用户报告：cron 微信派发能收到了 → 前 5 层 + R-OBS-001 全链路打通] |
...
| TC-AGENT-033-i | R-AGENT-033 | 源码扫描：`Run.kt::_handleMessage` 函数体。Bug A 修复（`setSessionVars`）只写当前线程 ThreadLocal，但下游 agent loop / tool dispatch 进入 `withContext(Dispatchers.IO)` 后切线程 → ThreadLocal 丢失 → `CronjobTools._originFromEnv` 拿不到值 → 微信触发的 cron job 落进 jobs.json 的 `origin=null` / `deliver=local`，cron 触发后只 `writeLocalChatNote`，IM 永远收不到。2026-06-24 用户报告 jobId `d137dd1dd6c0` 实测确认（cron.log line 22-27：`deliver mode=local originPlatform= originChatId=`，但创建路径是微信里 @ agent 建的）。修法：`setSessionVars` 之后用 `withContext(sessionContextElement()) { ... }` 包住整个 try 块下游逻辑（R-AGENT-045 已有 `sessionContextElement()` 工具函数），让 ThreadLocal 跨 dispatcher 传播；对齐 Python `contextvars.ContextVar` + `copy_context().run` 行为。 | 源码扫描：`Run.kt::_handleMessage` 函数体在 `setSessionVars(` 之后必须含 `withContext(sessionContextElement()` 字面值；该 `withContext` 块必须包住 `_runAgentLoop` / agent dispatch 调用（即 setSessionVars 字符 index < withContext(sessionContextElement index < agent loop call index）。 | unit-scan | `RunSessionVarsCrossDispatcherWiringTest#TC-AGENT-033-i _handleMessage wraps downstream in sessionContextElement` 🔴 |
| TC-AGENT-033-j | R-AGENT-033 | 源码扫描：`CronjobTools.kt::_originFromEnv` 函数体。`origin=null` 是个静默 fallback —— 上次出 bug 时 log 里完全看不到为什么 origin 是空的（platform 空? chatId 空? 都空?），只能靠 deliver=local 副作用倒推。加诊断 log：成功捕获 origin 时写 `i` 级日志 `origin captured platform=... chatId=... threadId=...`；missing 时写 `w` 级日志 `origin missing platform=... chatId=...`，让下次 cron→IM 链路断裂时一眼看到是 ThreadLocal 读到空还是 `_originFromEnv` 没被调用。 | 源码扫描：`CronjobTools.kt::_originFromEnv` 函数体必须含 `Log.` 或 `AppLogger.` 或类似 logger 字面值至少 2 次（成功路径 + missing 路径），且字面值含 `origin captured` 与 `origin missing` 两个 keyword 字符串。 | unit-scan | `CronjobOriginFromEnvDiagnosticLogWiringTest#TC-AGENT-033-j _originFromEnv logs capture/missing path` 🔴 |
...
| TC-AGENT-045-i-1 | R-AGENT-045 | 源码扫描 `MessageProcessingDelegate.kt::sendUserMessage()` | line ~521 `coroutineScope.launch(Dispatchers.IO) { ... }` 必须改为 `coroutineScope.launch(Dispatchers.IO + sessionContextElement()) { ... }`（同时 import `com.xiaomo.hermes.hermes.gateway.sessionContextElement`）—— service-scope `coroutineScope.launch` 默认 **不继承** caller 的 `CoroutineContext`，纯靠 `withContext(sessionContextElement())` 在 caller 侧包裹只能管到当前协程内部，跨过 service-scope `launch` 边界后 ThreadLocal 仍然丢。在 launch context 上 `+ sessionContextElement()` 才是 1:1 对齐 Python `copy_context().run(func)` 的"派生新协程时立即捕获 contextvars 快照"语义 | unit (源码扫描) | `MessageProcessingDelegateSessionContextTest#TC-AGENT-045-i-1 launches sendJob with sessionContextElement` 🟢 |
| TC-AGENT-045-i-2 | R-AGENT-045 | 行为测试：构造一个最简的 `coroutineScope: CoroutineScope`（`TestScope` + `StandardTestDispatcher`）；在源线程 `setSessionVars(platform="app", chatId="c-i-2")`；模拟 `MessageProcessingDelegate.sendUserMessage` 的 launch 模式 `coroutineScope.launch(Dispatchers.IO + sessionContextElement()) { getSessionEnv(...) }`；等 launch 完成 | launched 协程内部 `getSessionEnv("HERMES_SESSION_PLATFORM") == "app"` + `HERMES_SESSION_CHAT_ID == "c-i-2"` —— 守 launch context 上的 `+ sessionContextElement()` 真的把 ThreadLocal 快照打包了。否则即便源码扫描通过，运行时仍可能因 launch 实现 / element 实现回归丢失 | unit (behavior, kotlinx-coroutines-test) | `SessionContextElementTest#TC-AGENT-045-i-2 sessionContextElement propagates through launch context` 🟢 |
| TC-AGENT-045-i-3 | R-AGENT-045 | 源码扫描 `AIMessageManager.kt::sendMessage()` | 两处 `share(scope = scope, ...)` / `shareRevisable(scope = scope, ...)` 调用必须改成 `scope = CoroutineScope(scope.coroutineContext + sessionContextElement())` —— 静态 `AIMessageManager.scope = CoroutineScope(Dispatchers.IO + SupervisorJob())` 上的 `share()` 内部 `scope.launch { upstream.collect ... }` 是裸 launch，**不继承** caller 的 ThreadContextElement。cold `stream { }` 块里跑 agent loop + tool dispatch + `_originFromEnv()`，如果 producer 在没 session ThreadLocal 的线程上跑 → origin null。必须 import `com.xiaomo.hermes.hermes.gateway.sessionContextElement`，且两处 `scope =` 调用都要改 | unit (源码扫描) | `AIMessageManagerSessionContextTest#TC-AGENT-045-i-3 wraps share scope with sessionContextElement` 🔴 |
| TC-AGENT-045-i-4 | R-AGENT-045 | 源码扫描 `ExternalChatRequestExecutor.kt::prepareRequest()` | `sendParams` 必须额外注入 `__origin_platform=app` + `__origin_chat_id=<resolved>` 两个 `ToolParameter`（无论 createNewChat 分支还是既有 chat 分支）—— 这是 C-route 的源头：把 origin 作为**显式参数**穿过 5 层接口，绕过 service-scope `coroutineScope.launch { ... }` 砍掉 `CoroutineContext.Element` 的架构洞（见 `MessageCoordinationDelegate.kt:292`）。`sessionContextElement()` 路径只能管协程内嵌 `withContext`，跨过 `coroutineScope.launch { ... }` 边界后 ThreadLocal 仍丢，必须靠显式参数兜底 | unit (源码扫描) | `ExternalChatRequestExecutorSessionVarsTest#TC-AGENT-045-i-4 injects origin params into sendTool` 🔴 |
| TC-AGENT-045-i-5 | R-AGENT-045 | 源码扫描 `StandardChatManagerTool.kt::startMessageToAIStream()` | 必须从 `tool.parameters` 读 `__origin_platform` / `__origin_chat_id`，并把这两个值通过新增的 `originPlatformOverride` / `originChatIdOverride` 参数 forward 到两处 `core.sendUserMessage(...)` 调用 —— 工具层是 C-route 的"显式参数桥"第一段，这两值不能停留在 `AITool.parameters` 后被丢弃 | unit (源码扫描) | `StandardChatManagerToolOriginParamsTest#TC-AGENT-045-i-5 forwards origin params to ChatServiceCore` 🔴 |
| TC-AGENT-045-i-6 | R-AGENT-045 | 源码扫描 `ChatServiceCore.kt::sendUserMessage()` + `MessageCoordinationDelegate.kt::sendUserMessage()`/`sendMessageInternal()` + `MessageProcessingDelegate.kt::sendUserMessage()` | 4 个函数的签名必须新增 `originPlatformOverride: String? = null` + `originChatIdOverride: String? = null` 参数，且每层把这两个值原样 forward 到下一层调用点 —— 完整 5 层显式参数管道。`MessageCoordinationDelegate.sendUserMessage` 中 `chatIdOverride.isNullOrBlank()` 分支的 `coroutineScope.launch { sendMessageInternal(...) }`（line ~292）必须以 closure capture 方式把这两个值带进 launch 块 | unit (源码扫描) | `ChatServiceOriginParamsTest#TC-AGENT-045-i-6 chains origin params through 4-layer signatures` 🔴 |
| TC-AGENT-045-i-7 | R-AGENT-045 | 源码扫描 `MessageProcessingDelegate.kt::sendUserMessage()` line ~530 launch 块第一行 | 进入 `coroutineScope.launch(Dispatchers.IO + sessionContextElement()) { ... }` 后**第一行必须**调 `setSessionVars(platform = originPlatformOverride.orEmpty(), chatId = originChatIdOverride.orEmpty())` + `setCronAutoDeliverVars(...)`（且仅当 `originPlatformOverride != null`）—— 在 launch 体内**重新写入** ThreadLocal，把显式参数转成 ThreadLocal 让下游 `_originFromEnv()` 可读。launch 块尾部 finally 调 `clearSessionVars()` + `clearCronAutoDeliverVars()` 防泄漏。同时 import `setSessionVars` / `setCronAutoDeliverVars` / `clearSessionVars` / `clearCronAutoDeliverVars` | unit (源码扫描) | `MessageProcessingDelegateSessionContextTest#TC-AGENT-045-i-7 writes session vars from origin params inside launch` 🔴 |
...
./gradlew :app:testDebugUnitTest --tests "com.ai.assistance.operit.services.core.MessageProcessingDelegateSessionContextTest"
./gradlew :app:testDebugUnitTest --tests "com.ai.assistance.operit.core.chat.AIMessageManagerSessionContextTest"
./gradlew :app:testDebugUnitTest --tests "com.ai.assistance.operit.core.tools.defaultTool.standard.StandardChatManagerToolOriginParamsTest"
./gradlew :app:testDebugUnitTest --tests "com.ai.assistance.operit.services.ChatServiceOriginParamsTest"
```
...
## 域 CRON — 分段连发：按 agent loop 轮次拆消息（2026-06-25 新功能；R-CRON-STREAMING-001）

**背景**：r3 APK 后用户在微信收到了 cron 触发的喝水提醒，但 agent 整段回复（思考 + 调工具 + 最后总结）累一次性才发出来，体感等很久。需求是"app 对话框架看到的——每出现一句 assistant 回复就发一次微信"。技术上利用既有 `AgentEventBus.AssistantDelta(text, turn)`（`AgentLoop.kt:582` 已 emit；`EnhancedAIService.kt:1167` 同步桥接到 bus，key = `taskIdValue` = `chatId`），在 `CronAgentRunner.run` 入口订阅 bus 旁路，每轮 `AssistantDelta` 来一条就 `HermesReplyMarkupStripper.strip(...)` + `gateway.dispatchOutgoing(...)`，主路 `responseBuilder` 累加 + `deliver(...)` **不动**（只在 IM 派发处加去重保护，避免主路再发一次）。微信群聊 `chatId.endsWith("@chatroom")` 强制走旁路退回（不订阅、走主路一次发清），避免群里刷屏。

**与既有功能的零回归承诺**：
- **插话 / steer**（R-AGENT-036/037）：走 `inbound` 路径 → `_pendingSteer` → `_applyPendingSteerToToolResults`，与 `AgentEventBus` 完全独立，不受影响。
- **cron 定时器接收**（R-AGENT-031/033/043）：`CronExactAlarmReceiver.onReceive` → `CronAgentRunner.run` 入口路径不变。
- **既有 sanitize**（TC-CRON-SANITIZE-a..d）：`HermesReplyMarkupStripper.strip` 既有 2 个调用点（`CronAgentRunner.kt:178` + `HermesGatewayController.kt:654`）原样保留；旁路是**第三个调用点**，不替换不删除。
- **app 自己的聊天界面**：不订阅 bus 旁路（旁路只在 `CronAgentRunner.run` 里 install），UI 仍按 chunk 流走。

| ID | 关联 R-ID | 行为描述（输入 / 期望输出） | 测试断言 | 测试类型 | 测试方法 / 状态 |
|---|---|---|---|---|---|
| TC-CRON-STREAMING-a | R-CRON-STREAMING-001 | `CronAgentRunner.run` 在 `responseStream.collect`（或等价的 stream 消费）**之前**必须订阅 `AgentEventBus.events`，过滤 key = 当前 `chatId`（与 `EnhancedAIService` 写 bus 用的 `taskIdValue` 对齐），且只 react `AgentEvent.AssistantDelta`。该订阅必须在 coroutine scope 里以 `launch` / `collect` 旁路启动，不阻塞主流 chunk 累加。 | 源码扫描 `CronAgentRunner.kt`：`run` 函数体必须含 `AgentEventBus` + `AssistantDelta` 字面值；`AssistantDelta` 字面值的字符 index 必须出现在 `responseStream` / `responseBuilder.append(` 字面值之前（按字符 index 比较）；必须含 `launch` 字面值证明旁路 coroutine 启动；必须含 `chatId` 在订阅 filter 表达式里（用于 key match）。 | unit-scan | `CronStreamingDispatchWiringTest#TC-CRON-STREAMING-a run subscribes to AssistantDelta bus before stream collect` 🟢 |
| TC-CRON-STREAMING-b | R-CRON-STREAMING-001 | 每条 `AssistantDelta(text, turn)` 收到后必须：(1) `HermesReplyMarkupStripper.strip(text)` 剥 markup；(2) `trim()` 后若非空才 dispatch；(3) 调 `gateway.dispatchOutgoing(platform = originPlatform, chatId = originChatId, text = stripped, threadId = originThreadId)`；(4) dispatch 失败时仅 `CronFileLogger.w(...)` 不抛、不中断 agent loop。 | 源码扫描 `CronAgentRunner.kt` 订阅旁路块：必须含 `HermesReplyMarkupStripper.strip(` 字面值；必须含 `dispatchOutgoing(` 字面值；必须含 `isNotEmpty()` 或 `isNotBlank()` 字面值（空字符串保护）；必须含 `CronFileLogger` + `try` / `catch` 字面值（错误吞掉）。 | unit-scan | `CronStreamingDispatchWiringTest#TC-CRON-STREAMING-b each AssistantDelta is stripped and dispatched and errors swallowed` 🟢 |
| TC-CRON-STREAMING-c | R-CRON-STREAMING-001 | 群聊回退：当 `originPlatform == "weixin"` 且 `originChatId.endsWith("@chatroom")` 时**不**订阅 bus 旁路（直接走主路 final `deliver(...)` 一次发清），避免群里 N 条连发刷屏。其他平台 / 私聊全部走旁路。 | 源码扫描 `CronAgentRunner.kt`：旁路订阅入口前必须含 `@chatroom` 字面值 + `endsWith(` 字面值；并且该条件必须用 if / when **跳过订阅**（旁路 launch 在该条件 false 分支或 guard 之内）。 | unit-scan | `CronStreamingDispatchWiringTest#TC-CRON-STREAMING-c weixin chatroom skips streaming sidecar` 🟢 |
| TC-CRON-STREAMING-d | R-CRON-STREAMING-001 | 顺序保证：旁路同一 chatId 内多条 `AssistantDelta` 必须串行 dispatch（用 `Mutex` 或 `Channel` 顺序 collect），不并发 dispatchOutgoing，避免 N 条消息在微信里乱序。 | 源码扫描 `CronAgentRunner.kt` 旁路块：必须含 `Mutex` 或 `Channel` 字面值；如果是 `Mutex` 必须含 `.withLock` 字面值；如果是 `Channel` 必须含 `.consumeEach` 或 `.collect` 字面值（顺序消费）。 | unit-scan | `CronStreamingDispatchWiringTest#TC-CRON-STREAMING-d streaming dispatch is sequential per chatId` 🟢 |
| TC-CRON-STREAMING-e | R-CRON-STREAMING-001 | 去重保护：旁路在第一次成功 dispatch 后必须把 runner 上一个 `streamingDeliveredAny` 标记置 true；`deliver(...)` 入口检测该标记为 true 时**跳过 IM dispatch**（不调 `gateway.dispatchOutgoing` / `gateway.deliverText`）但仍要 `writeLocalChatNote(...)` + `saveJobOutput(...)`，保证 jobs.json + 本地 chat 记录与往常一致。同时旁路在 agent loop 没产生任何 `AssistantDelta` 时（标记仍是 false）—— `deliver(...)` 必须走正常 IM 路径作为回退。 | 源码扫描 `CronAgentRunner.kt`：必须含 `streamingDeliveredAny` / `streamingDelivered` 类标记字面值（按字面值匹配，至少 1 处 set + 1 处 read）；`deliver` 函数体必须含该标记的 `if`（或等价）分支判断；该分支体必须含 `writeLocalChatNote(` 字面值且**不**含同分支内的 `dispatchOutgoing(` / `deliverText(` 字面值（即检测到标记为 true 后跳过 IM 调用）。 | unit-scan | `CronStreamingDispatchWiringTest#TC-CRON-STREAMING-e deliver skips IM dispatch when streaming sidecar already sent` 🟢 |
| TC-CRON-STREAMING-f | R-CRON-STREAMING-001 | **订阅时序保证**（2026-06-25 bugfix）：`AgentEventBus.events` 是 `SharedFlow(replay=0)`，`launch { collect {...} }` 只把 coroutine 入队，订阅注册是异步发生的。如果 `enhancedService.sendMessage(...)` 在订阅注册完成前触发 `HermesAgentLoop` 开始 emit `AssistantDelta`，所有早期事件会被丢弃 → 旁路 dispatch 0 条 → `deliver(...)` 走兜底主路把全部内容合成一条发出（用户报"不是一条条出来的，是全部一起出来"）。修法：旁路必须用 `onSubscription { ... }` 回调把"订阅已注册"信号传出（通过 `CompletableDeferred<Unit>`），主路在调用 `enhancedService.sendMessage(...)` 之前必须 `await()` 该信号。 | 源码扫描 `CronAgentRunner.kt`：必须含 `CompletableDeferred` 字面值（用于 ready signal）；必须含 `onSubscription` 字面值（用于触发 ready）；必须含 `.await()` 字面值；且 `.await()` 的字符 index 必须出现在 `enhancedService.sendMessage(` 字面值**之前**（按字符 index 比较，证明主路确实等了订阅再触发 agent）。 | unit-scan | `CronStreamingSubscriptionReadyWiringTest#TC-CRON-STREAMING-f run awaits sidecar subscription before triggering agent` 🟢 |
| TC-CRON-STREAMING-g | R-CRON-STREAMING-001 | **In-flight dispatch 不被 cancel 砍**（2026-06-25 第三次 bugfix，真凶定位）：主路 `responseStream.collect` 跑完后立即 `sidecarJob?.cancel()` 会把 sidecar 协程里**正在飞**的 `gateway.dispatchOutgoing(...)`（OkHttp 网络请求）作为子协程一并 cancel，抛 `StandaloneCoroutine was cancelled`，导致 `dispatchOutgoing` 返回 ok=false。结果：sidecar 计数器 `dispatchCalls=1` 但 `dispatchSuccess=0`，`streamingDelivered=false`，`deliver(...)` 走主路兜底把整段合并为一条发出（用户报"全部一起出来"）。根因日志证据：`streaming dispatch threw ... reason=StandaloneCoroutine was cancelled`。修法：(a) `dispatchOutgoing(...)` 必须包在 `withContext(NonCancellable) { ... }` 内，确保即使外层 sidecarJob 被 cancel，已经在飞的网络请求也能跑完拿到结果；(b) 主路 cancel 之前应给 sidecar 一个收尾机会（如 `withTimeoutOrNull` join 等待 in-flight 完成）；(c) sidecar 跑到任何一条 dispatch 完成（无论成功失败）后再退出，不应被外层取消打断 in-flight 调用。 | 源码扫描 `CronAgentRunner.kt`：(1) sidecar 块内必须含 `NonCancellable` 字面值（dispatch 保护）；(2) `NonCancellable` 必须与 `withContext(` 字面值在同一函数体内（即真包了 dispatch，不是空写）；(3) `withContext(NonCancellable` 字面值的字符 index 必须在 `dispatchOutgoing(` 字面值**之前**（按字符 index 比较，证明确实包住 dispatch 调用）；(4) sidecar cancel 调用前必须有等待 in-flight 完成的机制：`responseStream.collect { ... }` 之后到 `sidecarJob?.cancel()` 之间必须含 `withTimeoutOrNull(` 或 `.join()` 字面值（证明给了 in-flight dispatch 收尾的时间窗口）。 | unit-scan | `CronStreamingDispatchNotCancelledWiringTest#TC-CRON-STREAMING-g sidecar dispatch is non-cancellable and main path drains before cancel` 🟢 |
| TC-CRON-STREAMING-manual | R-CRON-STREAMING-001 | 端到端手测：装新 APK，在微信**私聊**里跟 agent 说"5 分钟后提醒我喝水"，等到点。期望微信收到**多条独立消息**（每轮 agent reply 一条 bubble，思考/工具调用轮次跳过），最后一条 = final 总结。同样在微信**群聊**（`@chatroom`）里测试，期望仍是**一条整段消息**（不刷屏）。同时跑 `tail -100 /sdcard/Download/Hermes/cron_logs/cron.log`：必须含每轮的 `streaming dispatch turn=<n>` 类日志。app 自己的聊天界面**不受影响**（按 chunk 流原样显示）。 | (1) 微信私聊：≥2 条独立消息（agent 实际产生多轮时），每条都已剥 `<think>` / `<tool>` / `<tool_result>` / `<status>` 标签。(2) 微信群聊：1 条整段消息。(3) `cron.log` 含 `streaming dispatch` 字面值（每轮一行）。(4) app 自己的聊天页面行为不变。(5) 插话 `/steer`：私聊里旁路连发过程中触发 `/steer`，agent loop 必须能正常切到 steer 路径（既有 R-AGENT-036/037 行为不破）。 | manual / E2E | `(no unit test; manual verification required)` 🔴 |
| TC-CRON-STREAMING-h | R-CRON-STREAMING-002 | **Prompt 引导层**：`CronAgentRunner` 构造的 `wrappedPrompt` 必须在既有 `CRON_CONTEXT_PREFIX_EN/CN` 之后追加 **bilingual multi-message hint**——告诉 agent "如果回复内容自然分多段，请用空行 (`\n\n`) 分段，IM 端会按段落分多条发"。该 hint 必须是 file-scope 常量（与既有 prefix 同位置），便于单元测试断言。 | 源码扫描 `CronAgentRunner.kt`：(1) 必须存在 bilingual hint 常量（命名含 `MULTI_MESSAGE_HINT` 或同义字面值，含 `\n\n` 描述 + 含 `blank line` / `空行` 双语关键字字面值）；(2) `wrappedPrompt` 构造（`buildString` 或等价）函数体中必须**同时**含 `CRON_CONTEXT_PREFIX_EN` + `CRON_CONTEXT_PREFIX_CN` + 该 hint 常量字面值，三者按序追加（hint 字符 index 在 `rawPrompt` append 之前，证明 hint 落进了 system context 区域而不是污染了 user prompt）。 | unit-scan | `CronStreamingParagraphSplitWiringTest#TC-CRON-STREAMING-h wrappedPrompt contains bilingual multi-message hint` 🟢 |
| TC-CRON-STREAMING-i | R-CRON-STREAMING-002 | **Sidecar 段落兜底切片**：sidecar 收到 `AssistantDelta` → 剥 markup → trim → 用 `Regex` 按**连续空行**（`\n\s*\n+` 或等价 `\\R\\s*\\R+`）分割得到 `paragraphs`；对每段非空 `paragraph` 单独 `gateway.dispatchOutgoing(...)`；空段过滤；保留 R-CRON-STREAMING-001 的 `dispatchMutex.withLock` 顺序串行。单段情况下（`paragraphs.size == 1`）行为与 R-CRON-STREAMING-001 字节级等价（1 条 dispatch）。 | 源码扫描 `CronAgentRunner.kt` sidecar 块（`AssistantDelta` 关键字之后到 `run()` 结尾）：(1) 必须含 `Regex(` 或 `.split(` 字面值（或经由 file-scope `PARAGRAPH_REGEX` 常量引入）；(2) 必须含连续空行 pattern（`\\n\\s*\\n` / `\\R\\s*\\R` / `(?:\\r?\\n){2,}` 任一字面值，inline 在 sidecar 或 `PARAGRAPH_REGEX` 常量里）；(3) 段落迭代必须含 `forEach` / `for (` / `.map` 之一遍历后再 `dispatchOutgoing(`；(4) `dispatchMutex.withLock` 字面值仍存在；(5) `isNotBlank()` / `isNotEmpty()` 空段过滤字面值仍存在。 | unit-scan | `CronStreamingParagraphSplitWiringTest#TC-CRON-STREAMING-i sidecar splits AssistantDelta by blank lines before dispatch` 🟢 |
| TC-CRON-STREAMING-j | R-CRON-STREAMING-002 | **段间延迟**：段落之间 sidecar 必须 `delay(INTER_PARAGRAPH_DELAY_MS)`（常量 ≥ 150ms），避免微信短时高频限流；该常量必须是 file-scope `const val` / top-level 常量，值 ≥ 150 便于单元测试可读。 | 源码扫描 `CronAgentRunner.kt`：(1) 必须存在常量声明 `INTER_PARAGRAPH_DELAY_MS`（按字面值匹配）；(2) 该常量值字面值必须 `>= 150`（regex `INTER_PARAGRAPH_DELAY_MS\s*[:=]\s*(\d+)L?` 抓数字 ≥ 150）；(3) sidecar 块内必须含 `delay(INTER_PARAGRAPH_DELAY_MS` 字面值（确实用上）。 | unit-scan | `CronStreamingParagraphSplitWiringTest#TC-CRON-STREAMING-j inter-paragraph delay constant is used between dispatches` 🟢 |
| TC-CRON-STREAMING-k | R-CRON-STREAMING-002 | **可观测性扩展**：sidecar 计数器扩 1 个 `sidecarParagraphDispatches`（每段 dispatch 一次 +1，与既有 `dispatchCalls` / `dispatchSuccess` 同位置维护）；`streaming sidecar summary` 行 / `streaming AssistantDelta` 行必须落 `paragraphCount=<n>` 字面值，方便排查"几 turn × 几段"。 | 源码扫描 `CronAgentRunner.kt`：(1) 必须含 `sidecarParagraphDispatches` 字面值（计数器名）；(2) 必须含 `paragraphCount=` 字面值在 `CronFileLogger` 调用相关上下文里（同一函数体内）；(3) sidecar 块内 `sidecarParagraphDispatches` 至少一处自增（按字面值匹配 `sidecarParagraphDispatches.incrementAndGet()` / `++` / `+= 1`）。 | unit-scan | `CronStreamingParagraphSplitWiringTest#TC-CRON-STREAMING-k sidecar logs paragraph counter for observability` 🟢 |
| TC-CRON-STREAMING-l | R-AGENT-033 / R-AGENT-045 / R-CRON-STREAMING-001 | **session vars 必须用 wire chatId 不能用 resolvedChatId**（2026-07-07 bugfix；用户报"更新微信冒泡后计时器无法工作"，cron.log 显示 5 轮 sidecar dispatch 全 `dispatchSuccess=0`，微信 iLink 返回 `errcode=-3`）：`CronAgentRunner.run()` 在 `setSessionVars` / `setCronAutoDeliverVars` 两处必须把 `chatId` 参数传 **wire 形态的 `originChatId`**（来自 `origin.chat_id`，即 `wxid_xxx@im.wechat` 单段），而不是 `resolvedChatId`（`resolveChatId()` 返回的 Room chat UUID / `gw:...` 形态的 historyChatId，非 wire）。根因：`setCronAutoDeliverVars` 的 KDoc（`SessionContext.kt:100-107`）明确写"any `cronjob` tool call within this turn captures the inbound IM origin (platform / chat / thread)... letting the Scheduler later deliver the result back to that same chat"——`chat_id` 语义就是"能 dispatch 回去的 wire chatId"。若在 cron run 里传入 `resolvedChatId`，一旦 agent 在 cron run 期间调 `cronjob(action="create")`（例如续期提醒、创建关联任务），`_originFromEnv()` 抓到的 `HERMES_SESSION_CHAT_ID` 就是 Room UUID / 空串，写进新 job 的 `origin.chat_id` 从出生就是脏的，之后每次跑必挂 `errcode=-3`。`resolvedChatId` 只应在 sidecar `AgentEventBus` filter + `EnhancedAIService.sendMessage(chatId=...)`（Room 层）用，不该出现在 session vars 里。 | 源码扫描 `CronAgentRunner.kt` `run()` 函数体：(1) `setSessionVars(` 调用的 `chatId` 参数字面值等于 `originChatId`（按 regex `setSessionVars\s*\([^)]*chatId\s*=\s*originChatId` 匹配）；(2) `setCronAutoDeliverVars(` 调用的 `chatId` 参数字面值等于 `originChatId`（同 regex 匹配）；(3) **回归守卫**：`setSessionVars(...chatId = resolvedChatId` / `setCronAutoDeliverVars(...chatId = resolvedChatId` 字面值**不**存在（按 regex 严格排除）；(4) `resolvedChatId` 字面值仍存在（sidecar `AgentEventBus` filter + `sendMessage` 仍用它，不能误删）。 | unit-scan | `CronAgentRunnerWireChatIdWiringTest#TC-CRON-STREAMING-l session vars carry wire originChatId not resolvedChatId` 🟢 |
| TC-CRON-STREAMING-m | R-AGENT-033 / R-CRON-STREAMING-001 | **dispatch 前 chatId 卫生检查**（2026-07-07 防御深度）：sidecar dispatch 路径 + `deliver()` fallback 路径调 `gateway.dispatchOutgoing(platform, chatId=..., ...)` 之前，必须做一次 chatId 形态卫生检查——若 `chatId` 包含 `gw:` 前缀或多个 `@im.wechat` 段（regex `gw:` 匹配 startsWith，或 `@im\\.wechat` 出现次数 > 1 视为 wire 契约破坏），必须 **跳过 dispatch 并落 `CronFileLogger.e(...)` 报警行**（含"dirty chatId"或"invalid wire chatId"字面值 + 原始 chatId 值），不要把脏数据打到 `WeixinAdapter.send` 让微信 iLink 返 `errcode=-3` 才发现。这是防御深度，即使上游 `_handleMessage` / `_originFromEnv` 都对了，本层也能拒绝已知不合法的 wire 值。 | 源码扫描 `CronAgentRunner.kt`：(1) 存在一个私有辅助函数（含 `wireChatId` / `sanitize` / `isWireChatId` 类命名）或 inline 校验块，被 sidecar dispatch 与 `deliver()` fallback **两处**都引用；(2) 校验逻辑字面值含 `gw:` + `startsWith(` 字面值（前缀检查）；(3) 校验逻辑字面值含 `@im.wechat` + `count(` / `.split(` 类字面值（多段检查）；(4) 校验失败分支必须含 `CronFileLogger.e` 或 `CronFileLogger.w` 字面值 + "dirty" / "invalid" 类字面值（报警落日志）；(5) 校验失败分支必须**跳过** `dispatchOutgoing(` 调用（分支体内无 `dispatchOutgoing(` 字面值）。 | unit-scan | `CronAgentRunnerWireChatIdWiringTest#TC-CRON-STREAMING-m dispatch sanitizes chatId before adapter call` 🟢 |
| TC-CRON-STREAMING-002-manual | R-CRON-STREAMING-002 | **端到端手测**：装新 APK，在微信**私聊**里跟 agent 说"3 分钟后提醒我喝水，分多条消息说"，等触发。期望微信收到 **≥ 2 条独立消息**（按段落切片），段间间隔约 150–500ms。然后在群聊（`@chatroom`）里跑同样 prompt → 仍是 1 条整段（群聊回退）。`cat /sdcard/Download/Hermes/cron_logs/cron.log` 必须含 `paragraphCount=<n>` 字面值。app 内对话框 / `writeLocalChatNote` / `saveJobOutput` 落盘结果不变。 | (1) 微信私聊：≥ 2 条独立消息；(2) 微信群聊：1 条整段；(3) `cron.log` 含 `paragraphCount=` 字面值；(4) app 内对话不重复 / 不丢失；(5) 单段路径（agent 回复只有 1 段，无空行）：仍是 1 条（与 R-CRON-STREAMING-001 等价，无回归）。 | manual / E2E | `(no unit test; manual verification required)` 🔴 |
| TC-CRON-DIAG-001-a | R-CRON-DIAG-001 | **缺文件路径**：cron.log 不存在或不可读时，`diagnose_cron_streaming` executor 返回 `ToolResult(success=false)`，`error` 字段非空且含 "cron.log" 字面值，不抛异常、不崩溃。 | mock：`logFileOverride` 指向不存在的 path → invoke → `success==false && error.contains("cron.log") && result is StringResultData` 且 markdown 不含 rule 编号。 | unit | `CronDiagnosticToolsTest#TC-CRON-DIAG-001-a missing cron log returns success=false with descriptive error` 🟢 |
| TC-CRON-DIAG-001-b | R-CRON-DIAG-001 | **Rule#1 no agent run start**：cron.log 内容不含 `agent run start jobId=` 字面值时，命中 rule#1，markdown 含 "rule#1" + "cron.log 里没有该 job 的运行痕迹" 字面值。 | mock：cron.log 内容只有 "2026-06-25 12:00:00.000 I/Other: unrelated line\n" → invoke → markdown contains "rule#1" + "运行痕迹"。 | unit | `CronDiagnosticToolsTest#TC-CRON-DIAG-001-b absence of agent run start hits rule#1` 🟢 |
| TC-CRON-DIAG-001-c | R-CRON-DIAG-001 | **Rule#2 no sidecar summary**：日志有 `agent run start jobId=x` 但无 `streaming sidecar summary jobId=x` 时，命中 rule#2，markdown 含 "rule#2" + "sidecar 协程没启动" 字面值。 | mock：cron.log 含 "agent run start jobId=x name='r' promptLen=10" 但无后续 sidecar summary 行 → invoke → markdown contains "rule#2" + "sidecar"。 | unit | `CronDiagnosticToolsTest#TC-CRON-DIAG-001-c missing sidecar summary hits rule#2` 🟢 |
| TC-CRON-DIAG-001-d | R-CRON-DIAG-001 | **Rule#3 zero dispatch**：sidecar summary 含 `dispatchCalls=0` 或 `chatIdMatched=0` 时，命中 rule#3，markdown 含 "rule#3" + "originPlatform" 或 "originChatId" 字面值。 | mock：cron.log 含 `streaming sidecar summary jobId=x totalEvents=5 chatIdMatched=0 assistantDeltas=0 dispatchCalls=0 paragraphDispatches=0 dispatchSuccess=0 streamingDelivered=false` → invoke → markdown contains "rule#3" + "originPlatform"。 | unit | `CronDiagnosticToolsTest#TC-CRON-DIAG-001-d zero dispatch hits rule#3` 🟢 |
| TC-CRON-DIAG-001-e | R-CRON-DIAG-001 | **Rule#4 single paragraph**：`paragraphDispatches==assistantDeltas` 且 `dispatchSuccess>0` 时（典型 `paragraphCount=1`），命中 rule#4，markdown 含 "rule#4" + "agent 单 turn 输出整段" 字面值。 | mock：cron.log 含 `streaming sidecar summary jobId=x totalEvents=3 chatIdMatched=3 assistantDeltas=1 dispatchCalls=1 paragraphDispatches=1 dispatchSuccess=1 streamingDelivered=true` → invoke → markdown contains "rule#4" + "整段"。 | unit | `CronDiagnosticToolsTest#TC-CRON-DIAG-001-e single paragraph hits rule#4` 🟢 |
| TC-CRON-DIAG-001-f | R-CRON-DIAG-001 | **Rule#5 dispatch failed**：`dispatchCalls > dispatchSuccess` 时，命中 rule#5，markdown 含 "rule#5" + "gateway adapter 派发失败" 字面值，且原始关键行包含一条 `streaming dispatch failed` 或 `streaming dispatch threw` 行（如果日志里有）。 | mock：cron.log 含 sidecar summary 行 `dispatchCalls=3 paragraphDispatches=3 dispatchSuccess=1` + 一条 `streaming dispatch failed jobId=x turn=2 reason=ok=false` → invoke → markdown contains "rule#5" + "派发失败"。 | unit | `CronDiagnosticToolsTest#TC-CRON-DIAG-001-f partial dispatch failure hits rule#5` 🟢 |
| TC-CRON-DIAG-001-g | R-CRON-DIAG-001 | **Rule#6 streamingDelivered=false mismatch**：`streamingDelivered=false` 但 `dispatchSuccess > 0` 时，命中 rule#6，markdown 含 "rule#6" + "状态机" 字面值。 | mock：sidecar summary `dispatchCalls=2 paragraphDispatches=2 dispatchSuccess=2 streamingDelivered=false` → invoke → markdown contains "rule#6" + "状态机"。 | unit | `CronDiagnosticToolsTest#TC-CRON-DIAG-001-g streamingDelivered mismatch hits rule#6` 🟢 |
| TC-CRON-DIAG-001-h | R-CRON-DIAG-001 | **Rule#7 empty response**：日志含 `(empty response)` 字面值时，命中 rule#7，markdown 含 "rule#7" + "strip" 字面值。 | mock：cron.log 含 `agent run start jobId=x ...` + `strip emptied output jobId=x preStripLen=200` + `(empty response)` → invoke → markdown contains "rule#7" + "strip"。 | unit | `CronDiagnosticToolsTest#TC-CRON-DIAG-001-h empty response hits rule#7` 🟢 |
| TC-CRON-DIAG-001-i | R-CRON-DIAG-001 | **Rule#8 healthy**：`dispatchSuccess >= 1 && streamingDelivered=true` 且其它字段都正常时，命中 rule#8，markdown 含 "rule#8" + "PASS" 或 "无异常" 字面值。 | mock：sidecar summary `chatIdMatched=4 assistantDeltas=3 dispatchCalls=3 paragraphDispatches=5 dispatchSuccess=5 streamingDelivered=true` → invoke → markdown contains "rule#8" + ("PASS" or "无异常")。 | unit | `CronDiagnosticToolsTest#TC-CRON-DIAG-001-i healthy run hits rule#8` 🟢 |
| TC-CRON-DIAG-001-j | R-CRON-DIAG-001 | **`job_id` 入参**：显式传 `job_id=x` 时，工具只分析 `agent run start jobId=x` 块；同 cron.log 内有另一个 jobId 的 sidecar summary 不影响结论。 | mock：cron.log 含两段：jobId=GOOD（rule#8 健康）和 jobId=BAD（rule#5 失败）。调 invoke with `job_id=BAD` → markdown 命中 rule#5 而非 rule#8。 | unit | `CronDiagnosticToolsTest#TC-CRON-DIAG-001-j job_id parameter selects the correct block` 🟢 |
| TC-CRON-DIAG-001-k | R-CRON-DIAG-001 | **`tail_kb` 入参**：工具默认只读尾部 256KB；`tail_kb=8` 时只读最后 8KB，前面的日志即使含 `agent run start` 也不应被解析。 | mock：构造一个 12KB 的 log，前 4KB 含 agent run start + healthy summary（rule#8 候选），后 8KB 只含 unrelated 行（rule#1 候选）。调 `tail_kb=8` → 命中 rule#1（"运行痕迹"）。 | unit | `CronDiagnosticToolsTest#TC-CRON-DIAG-001-k tail_kb parameter limits read window` 🟢 |
| TC-CRON-DIAG-001-l | R-CRON-DIAG-001 | **输出结构**：markdown 输出必须含固定章节：`## 诊断 jobId=`, `### 关键计数器`, `### 概率最大根因`, `### 建议下一步`, `### 原始关键行`，顺序固定。 | mock：任何一条命中规则的输入 → markdown 含上述 5 个章节标题字面值，按顺序。 | unit | `CronDiagnosticToolsTest#TC-CRON-DIAG-001-l output markdown has fixed section structure` 🟢 |
| TC-CRON-DIAG-001-manual | R-CRON-DIAG-001 | **端到端手测**：装新 APK，触发一个 cron job 让它跑完，在 app 内对 agent 说"诊断刚才那个 cron" → agent 应调一次 `diagnose_cron_streaming`（不带参数）；返回值含命中规则编号 + 关键计数器值；用户阅读 markdown 即知道是 prompt 失败 / adapter 失败 / 还是 healthy。 | (1) agent 实际调用了 `diagnose_cron_streaming` 工具；(2) 返回 markdown 5 章节齐全；(3) markdown 中至少一条候选 rule 编号 + 一行原始关键日志被回显；(4) 工具自身**不**往 cron.log 写新行（保持只读语义）。 | manual / E2E | `(no unit test; manual verification required)` 🔴 |

状态图例: 🔴 = 无测试（待落地） / 🟡 = 有测试未验证 / 🟢 = 已绿

---

## 域 GW — 正常 IM 对话每 turn 流式分段连发（2026-06-26 新功能；R-GW-STREAMING-001）

**背景**：R-CRON-STREAMING-001/002 已经让 cron 定时任务触发的微信回复按 agent loop 轮次 + 段落分多条发。但**正常 IM 对话**（用户在微信里直接 @bot / 私聊触发的 agent 调用）走的是**另一条链路**：`GatewayRunner._handleMessage`（`Run.kt:317`）→ `HermesGatewayController.runHermesAgent` → `ChatServiceCore.sendUserMessage` → `EnhancedAIService.sendMessage`，主路整段跑完后由 `GatewayRunner` 在 `Run.kt:545` 调 `deliveryRouter.deliverText(...)` 一次性 dispatch。结果是用户在微信里要等 60s 静默后一次性收到一坨长文，看不到 agent 是"正在帮你查天气..."→"查到了，正在分析..."→"最终结果"这种推进感。本域把 R-CRON-STREAMING 的 sidecar 思路外延到正常 IM 路径。

**架构总览**:
- 新建公共类 `AgentStreamingSidecar`（`app/.../hermes/gateway/AgentStreamingSidecar.kt`），把订阅 `AgentEventBus` → 过滤 `TaggedEvent.chatId == myChatId` → `AssistantDelta` strip → 按 `PARAGRAPH_REGEX` 切段 → `dispatchMutex.withLock` 串行 `dispatchOutgoing` → `delay(INTER_PARAGRAPH_DELAY_MS)` 的算法抽出来，由 cron + 正常 IM gateway **共用同一个类**。
- `HermesGatewayController.runHermesAgent` 在调 `ChatServiceCore.sendUserMessage` 前 `coroutineScope { launch { sidecar.collect() } }` 启 sidecar；sidecar 内用 `onSubscription { ready.complete() }` 保证订阅落地后再调 agent，避免 `SharedFlow(replay=0)` 竞态。
- 群聊 `@chatroom`（仅 weixin）跳过 sidecar，回退到主路整段 deliver。
- 重复派发 sentinel：`STREAMING_DELIVERED_SENTINEL`（`Run.kt:39` 旁，与 `INTERRUPTED_SENTINEL` 同位置）。sidecar 派发过任何一段后，`runHermesAgent` 返回该 sentinel；`GatewayRunner._handleMessage` 在 `Run.kt:545` 的 `deliveryRouter.deliverText(...)` 前检查 sentinel，命中则**跳过 deliver**（流式已发完）。
- 仅订阅 `AssistantDelta`，**不**订阅 `Thinking` / `ToolCall*`（避免泄漏中间状态、避免刷屏）。
- 常量复用：`INTER_PARAGRAPH_DELAY_MS=200L` / `PARAGRAPH_REGEX=Regex("""\R\s*\R+""")` / `MULTI_MESSAGE_HINT` 保留在 `CronAgentRunner.kt`（不动 R-CRON-STREAMING-002 既有 wiring 测试）；`HermesGatewayController.kt` 同 package 声明同值副本。两侧通过 `AgentStreamingSidecar` 构造函数注入这些值（不通过 file-scope 常量共享）。

**与既有功能的零回归承诺**：
- **R-CRON-STREAMING-001/002 既有行为**：cron 路径重构为复用 `AgentStreamingSidecar` 后，cron.log 字段名 / 顺序 / 行格式 diff 为空；段间延迟、段落切片、群聊回退、订阅竞态保护、`NonCancellable` 包裹、in-flight drain 全部保留。
- **R-AGENT-031/033/035/036/037/045 / R-OBS-001 / R-CRON-DIAG-001**：不动。
- **R-GW-001..R-GW-013**：不动（`dispatchOutgoing` 复用，不新增 outbound API；`deliveryRouter` 接口不变）。
- **app 自己的聊天界面**：sidecar 只 dispatch 到外部 IM gateway，**不**写 app 内聊天历史；app UI 仍按 chunk 流走，与 sidecar 接入前等价。

| ID | 关联 R-ID | 行为描述（输入 / 期望输出） | 测试断言 | 测试类型 | 测试方法 / 状态 |
|---|---|---|---|---|---|
| TC-GW-STREAMING-001-a | R-GW-STREAMING-001 | **公共 sidecar 类存在**（2026-06-27 v2 简化）：`app/src/main/java/com/ai/assistance/operit/hermes/gateway/AgentStreamingSidecar.kt` 文件必须存在，含 `class AgentStreamingSidecar` 声明，构造函数接收 `chatId: String` / `platform: String` / `dispatchOutgoing: suspend (String, String, String, String?) -> Boolean` 三个核心参数（由调用方注入）。v2 起**不再**接收 `paragraphRegex` / `interParagraphDelayMs`——sidecar 改为整段直发，每个 AssistantDelta 一次性下发一条 IM，不再拆段（原拆段方案因 mutex 串行 + 段间延时让 collector 严重滞后于 agent loop，真机表现是"微信延迟一坨爆发"，2026-06-27 撤回）。 | 源码扫描 `AgentStreamingSidecar.kt`：(1) `class AgentStreamingSidecar` 字面值存在；(2) 构造函数声明含 `chatId` + `platform` + `dispatchOutgoing` 三个参数字面值；(3) 该类在 `app/.../hermes/gateway/` 包下；(4) **回归守卫**：源码中**不**含 `paragraphRegex` 或 `interParagraphDelayMs` 字面值（按 regex 匹配排除——v2 已撤回拆段，构造参数不再含这两项）。 | unit-scan | `AgentStreamingSidecarShapeWiringTest#TC-GW-STREAMING-001-a class exposes required constructor surface` 🟡 |
| TC-GW-STREAMING-001-b | R-GW-STREAMING-001 | **订阅竞态保护**：`AgentStreamingSidecar` 必须用 `CompletableDeferred<Unit>` + `onSubscription { ready.complete(Unit) }` 保证订阅落地后才让调用方触发 agent。暴露 `suspend fun awaitReady()` 或等价 API 让调用方在调 `sendMessage` 之前 `await`。 | 源码扫描 `AgentStreamingSidecar.kt`：(1) 含 `CompletableDeferred` 字面值；(2) 含 `onSubscription` 字面值；(3) 含 `.complete(` 字面值（ready 信号触发）；(4) 暴露 `awaitReady` 或 `subscriptionReady` 公共 API（按名称匹配）。 | unit-scan | `AgentStreamingSidecarShapeWiringTest#TC-GW-STREAMING-001-b sidecar exposes subscription-ready signal` 🟢 |
| TC-GW-STREAMING-001-c | R-GW-STREAMING-001 | **过滤 + 整段直发**（2026-06-27 v2 简化）：sidecar `collect` 块必须订阅 `AgentEventBus.events`，过滤 `event.chatId == this.chatId`，仅处理 `AgentEvent.AssistantDelta` 类型（不处理 `Thinking` / `ToolCallStart` / `ToolCallEnd` / `Final` / `Error`）；剥 `HermesReplyMarkupStripper.strip(...)` → trim → 若非空则**整段一次**调 `dispatchOutgoing`，不拆段、不 mutex 串行、无段间 delay。每个 AssistantDelta = 一次 IM dispatch = 一条独立气泡，与 agent loop 的"一轮一条" 1:1 对应。想要更细粒度的多气泡分发，让 agent 主动调 R-GW-STREAMING-002 的 `send_message` 工具实现，不再由 sidecar 切段。 | 源码扫描 `AgentStreamingSidecar.kt`：(1) 含 `AgentEventBus.events` 字面值；(2) 含 `AssistantDelta` 字面值，但**不**含 `ToolCallStart` / `ToolCallEnd` 字面值；(3) 含 `HermesReplyMarkupStripper.strip` 字面值；(4) 含 `dispatchOutgoing(platform` 调用字面值（真实派发点）；(5) **回归守卫**：源码中**不**含 `paragraphRegex` / `dispatchMutex` / `.withLock` / `.split(paragraphRegex` 字面值（v2 已撤回拆段串行）；(6) **回归守卫**：源码中**不**含 `delay(interParagraphDelayMs` 字面值（v2 已撤回段间延时）。 | unit-scan | `AgentStreamingSidecarShapeWiringTest#TC-GW-STREAMING-001-c sidecar filters AssistantDelta and dispatches as one IM message per turn` 🟡 |
| TC-GW-STREAMING-001-d | R-GW-STREAMING-001 | **NonCancellable 保护 in-flight dispatch**：与 R-CRON-STREAMING-001 (TC-CRON-STREAMING-g) 同样的修法——`dispatchOutgoing(...)` 必须包在 `withContext(NonCancellable) { ... }` 内，确保外层 cancel 不砍 in-flight 网络请求。 | 源码扫描 `AgentStreamingSidecar.kt`：(1) 含 `NonCancellable` 字面值；(2) 含 `withContext(` 字面值且 `NonCancellable` 在同一表达式内；(3) `withContext(NonCancellable` 的字符 index 在 `dispatchOutgoing(` 字面值**之前**（按字符 index 比较，证明确实包住调用）。 | unit-scan | `AgentStreamingSidecarShapeWiringTest#TC-GW-STREAMING-001-d dispatch is wrapped in NonCancellable` 🟢 |
| TC-GW-STREAMING-001-e | R-GW-STREAMING-001 | **失败语义**：单段 dispatch 失败仅记日志（不抛、不中断后续段落）；sidecar 内部至少有一个 try/catch 包住 dispatch；任何 `CancellationException` 必须 rethrow（结构化并发要求），其他异常 swallow + log。 | 源码扫描 `AgentStreamingSidecar.kt`：(1) 含 `try` + `catch` 字面值在 sidecar collect 块内；(2) 含 `CancellationException` 字面值 + `throw` 字面值（rethrow）；(3) 含 `GatewayFileLogger` 或 `AppLogger` 类日志调用字面值（错误落日志而非吞掉）。 | unit-scan | `AgentStreamingSidecarShapeWiringTest#TC-GW-STREAMING-001-e dispatch failures are swallowed but CancellationException is rethrown` 🟢 |
| TC-GW-STREAMING-001-f | R-GW-STREAMING-001 | **`STREAMING_DELIVERED_SENTINEL` 常量在 `Run.kt`**：`hermes-android/.../gateway/Run.kt` 的 `GatewayRunner` companion object 必须声明 `const val STREAMING_DELIVERED_SENTINEL: String`，值非空且与 `INTERRUPTED_SENTINEL` 等价位置（同 companion object 内）。值字面值必须不可能在自然语言里出现（含 `\u0000` 类 control char 或 `__STREAMING_DELIVERED__` 类 marker）。 | 源码扫描 `hermes-android/.../gateway/Run.kt`：(1) 含 `STREAMING_DELIVERED_SENTINEL` 字面值；(2) 含 `const val STREAMING_DELIVERED_SENTINEL` 字面值（声明）；(3) 该声明所在行的字符 index 距 `INTERRUPTED_SENTINEL` 字面值的 index 不超过 1500 字符（即两者在同一 companion object 块内，便于运维 grep）；(4) 该常量值字面值含 `__STREAMING_DELIVERED__` 或 `\u0000`（一种 marker pattern）。 | unit-scan | `GatewayRunnerStreamingSentinelWiringTest#TC-GW-STREAMING-001-f GatewayRunner declares STREAMING_DELIVERED_SENTINEL` 🟢 |
| TC-GW-STREAMING-001-g | R-GW-STREAMING-001 | **`GatewayRunner` 跳过 deliver**：`_handleMessage` 在 `Run.kt:545` 的 `deliveryRouter.deliverText(...)` 调用前必须有 `if (responseText == STREAMING_DELIVERED_SENTINEL) { ... return / skip ... }` 类守卫，命中则不调 `deliverText`。与既有 `responseText != INTERRUPTED_SENTINEL`（`Run.kt:509`）属同一函数体内的另一道闸门，**不**替换它。 | 源码扫描 `hermes-android/.../gateway/Run.kt` `_handleMessage` 函数体：(1) 含 `STREAMING_DELIVERED_SENTINEL` 字面值（被 `_handleMessage` 引用，证明守卫真的在主路上）；(2) 该字面值出现在 `deliveryRouter.deliverText(` 字面值**之前**（按字符 index）；(3) `INTERRUPTED_SENTINEL != responseText` 或 `responseText != INTERRUPTED_SENTINEL` 字面值仍存在（既有守卫不被破坏）。 | unit-scan | `GatewayRunnerStreamingSentinelWiringTest#TC-GW-STREAMING-001-g _handleMessage skips deliverText when sentinel returned` 🟢 |
| TC-GW-STREAMING-001-h | R-GW-STREAMING-001 | **`HermesGatewayController.runHermesAgent` 接入 sidecar**：函数体必须 `import com.ai.assistance.operit.hermes.gateway.AgentStreamingSidecar`，并构造一个 `AgentStreamingSidecar` 实例，在调 `ChatServiceCore.sendUserMessage` 或等价 agent 触发前 `awaitReady`。sidecar 派发过任何一段后，`runHermesAgent` 返回 `STREAMING_DELIVERED_SENTINEL`；否则返回原文（兜底）。 | 源码扫描 `HermesGatewayController.kt` `runHermesAgent` 函数体：(1) 含 `AgentStreamingSidecar` 字面值（类实例化或类型引用）；(2) 含 `STREAMING_DELIVERED_SENTINEL` 字面值（返回值引用，注意需要跨包 import 到 Run.kt 的 sentinel，或本地常量值等价）；(3) 含 `awaitReady` 或 `subscriptionReady` 字面值；(4) `awaitReady` 字面值的字符 index 在 `sendUserMessage` 或 agent 触发字面值**之前**（按字符 index 比较）。 | unit-scan | `HermesGatewayControllerStreamingWiringTest#TC-GW-STREAMING-001-h runHermesAgent constructs and drives AgentStreamingSidecar` + `…wasDelivered gates STREAMING_DELIVERED_SENTINEL return` 🟢 |
| TC-GW-STREAMING-001-i | R-GW-STREAMING-001 | **群聊回退**：`runHermesAgent` 在 sidecar 启动前必须检查 `platform == "weixin" && chatId.endsWith("@chatroom")`，命中则**不启动 sidecar**（直接走原有整段 deliver 路径，返回完整文本而非 sentinel）。 | 源码扫描 `HermesGatewayController.kt` `runHermesAgent` 函数体：(1) 含 `@chatroom` 字面值；(2) 含 `endsWith(` 字面值；(3) `AgentStreamingSidecar` 实例化或 `awaitReady` 调用在该条件的 false 分支内（即条件 true 时**跳过** sidecar 启动）—— 通过断言 `@chatroom` 字面值出现在 `AgentStreamingSidecar(` 字面值**之前**（即守卫先于实例化）来近似验证。 | unit-scan | `HermesGatewayControllerStreamingWiringTest#TC-GW-STREAMING-001-i group chats skip the streaming sidecar` 🟢 |
| TC-GW-STREAMING-001-j | R-GW-STREAMING-001 | **MULTI_MESSAGE_HINT 通过 SystemPromptComposeHook 注入**（v2.1，2026-06-26 修正）：Python 上游 `ephemeral_system_prompt` 模式 —— hint 追加到 `role: "system"` 消息，**不写进 user message**、**不持久化 Room ChatMessage**、**不污染 history**。Android 侧用现成的 `SystemPromptComposeHook`（`PromptHookRegistry.kt:53`）：`runHermesAgent` 在调 `core.sendUserMessage` 之前 `PromptHookRegistry.registerSystemPromptComposeHook(...)` 注册一个匿名 hook，hook 内**直接**返回 `PromptHookMutation(systemPrompt = base + "\n\n" + MULTI_MESSAGE_HINT)`（**不**做 `context.chatId == historyChatId` 过滤——v2 用过这个过滤，但 `SystemPromptConfig.getSystemPromptWithCustomPrompts(...)` 不会把 chatId 透传进 `PromptHookContext`，导致 `context.chatId` 恒为 null、过滤恒拒、hint 永不注入，真机表现就是"回复还是一坨"的 bug）；`finally` 块内 `unregisterSystemPromptComposeHook(hookId)` 防泄漏，gateway-scoping 由 `try { register } finally { unregister }` 的窗口承担，不再依赖 hook 内 chatId 比较。**不再**用 `messageTextOverride = wrappedText` 把 hint 塞 user text（v1 实现的架构错误：会持久化为 ChatMessage 并跨轮污染上下文）。常量副本仍按 `HermesGatewayController.kt` 同 package 独立声明（不与 `CronAgentRunner` 耦合）。 | 源码扫描 `HermesGatewayController.kt`：(1) 含 `blank line` 字面值；(2) 含 `空行` 字面值；(3) 含 `MULTI_MESSAGE_HINT` 或同义常量名字面值；(4) `runHermesAgent` 函数体内含 `registerSystemPromptComposeHook(` 字面值；(5) 含 `PromptHookMutation` 字面值（或 `SystemPromptComposeHook` 接口引用）；(6) 含 `unregisterSystemPromptComposeHook(` 字面值；(7) 含 `finally` 字面值（保证清理）；(8) **不**得含 `messageTextOverride = wrappedText` / `messageTextOverride = buildString` —— 用户文本路径必须保持 `messageTextOverride = text`（按 regex 匹配排除）；(9) **回归守卫（v2.1，2026-06-26）**：**不**得含 `context.chatId != historyChatId` 或 `context.chatId == historyChatId` 字面值（按 regex 匹配排除）—— hook 必须无条件返回 mutation，scoping 由 try/finally 窗口承担。 | unit-scan | `HermesGatewayControllerStreamingWiringTest#TC-GW-STREAMING-001-j runHermesAgent injects bilingual multi-message hint via SystemPromptComposeHook` 🟢 |
| TC-GW-STREAMING-001-k | R-GW-STREAMING-001 | **可观测性**：sidecar 落 `GatewayFileLogger` 行含 `streaming dispatch turn=` / `paragraphIdx=` / `paragraphCount=` 类字面值（与 R-CRON-STREAMING 字段命名对齐，便于 R-CRON-DIAG 同款诊断思路外延到 gateway 路径）。 | 源码扫描 `AgentStreamingSidecar.kt`：(1) 含 `GatewayFileLogger` 字面值（不要用 CronFileLogger —— gateway 路径走 GatewayFileLogger）；(2) 含 `streaming dispatch turn=` 字面值；(3) 含 `paragraphIdx=` 或 `paragraphCount=` 字面值（任一）。 | unit-scan | `AgentStreamingSidecarShapeWiringTest#TC-GW-STREAMING-001-k sidecar emits gateway log lines with required fields` 🟢 |
| TC-GW-STREAMING-001-l | R-GW-STREAMING-001 | **cron 路径重构后仍引用公共 sidecar**：重构 `CronAgentRunner.kt` 复用 `AgentStreamingSidecar` 之后，`run()` 函数体内必须含 `AgentStreamingSidecar` 字面值（证明确实复用了公共类）。同时既有 file-scope 常量 `MULTI_MESSAGE_HINT` / `INTER_PARAGRAPH_DELAY_MS=200L` / `PARAGRAPH_REGEX` 仍保留（保护 R-CRON-STREAMING-002 既有 wiring 测试 grep 这些字面值）。cron sidecar 通过把这三个常量传给 `AgentStreamingSidecar` 构造函数实现复用。 | 源码扫描 `CronAgentRunner.kt`：(1) `run()` 函数体内含 `AgentStreamingSidecar` 字面值；(2) `MULTI_MESSAGE_HINT` 字面值仍存在；(3) `INTER_PARAGRAPH_DELAY_MS` 字面值 + `= 200L`（或 `200L`）仍存在；(4) `PARAGRAPH_REGEX` 字面值仍存在；(5) 既有 `CronStreamingParagraphSplitWiringTest` / `CronStreamingSubscriptionReadyWiringTest` / `CronStreamingDispatchWiringTest` / `CronStreamingDispatchNotCancelledWiringTest` 全部仍绿（重构后跑一次确认）。 | unit-scan | `CronAgentRunnerSidecarRefactorWiringTest#TC-GW-STREAMING-001-l cron run() references AgentStreamingSidecar after refactor` 🔴 |
| TC-GW-STREAMING-001-m | R-GW-STREAMING-001 | **[REVERTED 2026-06-27]** ~~gateway 段间间隔常量 = 1500ms~~ —— v2 已撤销。原 m/n/o 三件套（1500ms 段间 + 2000ms 重试 + 800ms 预热）真机验证不 work：三个延时都塞在 `dispatchMutex.withLock` 内的串行 collector 里，让 sidecar 严重落后于 agent loop（一轮处理 ≥ 8s，期间后续轮的 AssistantDelta 在 SharedFlow buffer 里排队），用户在微信端看到"先沉默 N 秒再一次性爆发"。修法是釜底抽薪：sidecar 改整段直发，删拆段/mutex/3 个延时（见新 TC-001-c）。微信限流问题改由 agent 主动调 `send_message` 工具控制节奏（R-GW-STREAMING-002）。 | （不再扫描；测试方法已删） | unit-scan | `~~HermesGatewayControllerDelayConstantWiringTest~~ 已删` ⚪ |
| TC-GW-STREAMING-001-n | R-GW-STREAMING-001 | **[REVERTED 2026-06-27]** ~~段失败重试 1 次 + 2000ms backoff~~ —— v2 已撤销。同 m 项原因：retry 的 `delay(2000)` 在 mutex 内阻塞整个 collector。新设计放弃 sidecar 内重试——单次 `dispatchOutgoing(...)` 失败仅记日志（与原 TC-001-e 失败语义相符），不重试、不长延时。`send_message` 工具的失败处理由 agent loop 看到 `success=false` 后自行决定（重发或换措辞）。 | （不再扫描；测试方法已删） | unit-scan | `~~AgentStreamingSidecarRetryWiringTest~~ 已删` ⚪ |
| TC-GW-STREAMING-001-o | R-GW-STREAMING-001 | **[REVERTED 2026-06-27]** ~~首段预热 800ms~~ —— v2 已撤销。同 m 项原因：`delay(800)` 在 mutex 内阻塞整个 collector，让首轮派发慢 800ms，所有后续轮排队等待。新设计删除 firstDispatch 标志位 + 预热 delay，AssistantDelta 一到立即派发整段。 | （不再扫描；测试方法已删） | unit-scan | `~~AgentStreamingSidecarRetryWiringTest~~ 已删` ⚪ |
| TC-GW-STREAMING-001-p | R-GW-STREAMING-001 | **bus-tag chatId 与 wire chatId 必须分字段**（2026-06-27 bugfix）：真机日志显示 `WeixinAdapter.send(to_user_id=...)` 收到的是 `gw:weixin:wxid@im.wechat:wxid@im.wechat:wxid@im.wechat`（5-token 形式），微信 iLink 返回 `errcode=-3`（INVALID_PARAM）→ 流式派发全废，仅最后的非流式 fallback 命中（用户看到的"沉默后突然爆发"）。根因：`AgentStreamingSidecar` 构造函数只有一个 `chatId: String` 字段，同时承担"事件总线 tag 过滤"（需 5-token `historyChatId="gw:$sessionKey:$chatId"`）和"传给 IM adapter 的 wire chatId"（需 1-token 平台原生 chatId，例如微信的 `wxid@im.wechat`）两个语义。修法：构造函数拆成两个字段——`busTagChatId: String`（用于 `tagged.chatId == this.busTagChatId` 过滤）+ `wireChatId: String`（用于 `dispatchOutgoing(platform, wireChatId, ...)`）。`HermesGatewayController.runHermesAgent` 在构造 sidecar 时传 `busTagChatId = historyChatId, wireChatId = chatId`。**不**改 `historyChatId` 表达式（DB / event bus / GatewayOutboundRegistry 注册 key 仍需 5-token），**不**改 `WeixinAdapter.send`（adapter 契约本就是接 wire chatId）、**不**改 fallback / send_message 工具路径（已经传的是 clean chatId）。 | 源码扫描 `AgentStreamingSidecar.kt`：(1) 构造函数同时声明 `busTagChatId` 和 `wireChatId` 两个字面值；(2) **不**得只声明 `chatId` 单字段——`val chatId:` 字面值不存在（按 regex 严格匹配排除）；(3) filter 块内含 `busTagChatId` 字面值（即 `tagged.chatId == this@AgentStreamingSidecar.busTagChatId` 或等价表达式）；(4) `dispatchOutgoing(platform, wireChatId` 字面值存在（真实派发点用 wire chatId 不是 bus tag）。源码扫描 `HermesGatewayController.kt:runHermesAgent`：(5) 含 `busTagChatId = historyChatId` 或 `busTagChatId=historyChatId` 字面值；(6) 含 `wireChatId = chatId` 或 `wireChatId=chatId` 字面值。 | unit-scan | `AgentStreamingSidecarShapeWiringTest#TC-GW-STREAMING-001-p sidecar splits busTagChatId and wireChatId` + `HermesGatewayControllerStreamingWiringTest#TC-GW-STREAMING-001-p controller passes historyChatId as busTag and platform chatId as wire` 🔴 |
| TC-GW-STREAMING-001-manual | R-GW-STREAMING-001 | **端到端手测**（2026-06-27 v2 简化）：装新 APK，在微信**私聊**发"帮我查下天气并提醒我带伞"或其他会调工具的多轮请求 → 微信收到 **N 条独立气泡**（按 agent loop 轮次 1:1，每轮 sidecar 整段一次下发；另外若 agent 主动调 `send_message` 工具还会插入更多气泡）。本体 app 与微信几乎**同步**显示——不再有"本体先显示一大堆、微信半天没动"的滞后。微信**群聊** `@chatroom` 同样请求仍是**整段一条**（回退路径）。`cat /sdcard/Download/Hermes/gateway_logs/gateway.log` 含 `streaming dispatch turn=` + `textLen=` 字面值（不再有 `paragraphIdx=` / `paragraphCount=`，因为不拆段）。同时确认（a）app 内对话页面行为不变；（b）cron 定时任务的 R-CRON-STREAMING-001/002 行为不变；（c）三个 §3 E2E 脚本仍全绿。 | (1) 微信私聊：与 agent loop 轮次 1:1 的独立气泡（不再延迟爆发）；(2) 微信群聊：1 条整段；(3) gateway.log 含 `streaming dispatch turn=` + `textLen=`；(4) app 内对话框不重复 / 不丢失；(5) cron 定时任务跑一遍 → 行为不变；(6) 三个 E2E 脚本全绿。 | manual / E2E | `(no unit test; manual verification required)` 🔴 |

状态图例: 🔴 = 无测试（待落地） / 🟡 = 有测试未验证 / 🟢 = 已绿 / ⚪ = 已撤销（[REVERTED]）

---

## 域 GW — gateway.log 按 run 保留最近两次（2026-06-27 调试辅助；TC-GW-DIAG-002）

**背景**：R-GW-STREAMING-001 v2 上线后真机仍报"延迟一坨爆发"。排查靠 `gateway.log`，但该文件历史按字节中点截断（`trimIfNeeded` 命中 2MB 时砍前半），会切到单次 run 中间，丢失上下文，定位困难。

**核心方案**：`GatewayFileLogger` 新增 `startRun(label)` / `endRun(label)` 两个公共 API。`HermesGatewayController.runHermesAgent` 在入口调 `startRun(historyChatId)`，在 3 个出口（normal / interrupted / early-out `/new`）调 `endRun(...)`。`startRun` 写入前先调内部 `trimToLastTwoRuns(file)` —— 扫描文件找到最后一个 `▶▶▶ RUN START` 标记，把它之前的内容全删，再追加新 START。**净效果**：磁盘上始终保留"上一次 run + 当前 run = 两次"，更早的全部丢弃。

**与既有功能的零回归承诺**：
- `log()` / `i()` / `w()` / `e()` / `d()` / `logSessionStart()` / `getLogFilePath()` 行为不变；`trimIfNeeded()` 2MB 兜底保留（作为 run 内单次过大时的安全网，不影响 run-边界裁剪）。
- `CronFileLogger` / `WeixinFileLogger` 完全不动（cron 和微信传输日志的 retention 另议）。
- `runHermesAgent` 的业务路径不动；仅在已有 3 个 END 点旁各追加一行 `endRun(...)`、入口加一行 `startRun(...)`。

| ID | 关联 R-ID | 行为描述（输入 / 期望输出） | 测试断言 | 测试类型 | 测试方法 / 状态 |
|---|---|---|---|---|---|
| TC-GW-DIAG-002-a | （调试辅助，无 R-ID） | **`GatewayFileLogger` 暴露 `startRun` / `endRun` 公共 API**，并定义 `▶▶▶ RUN START` / `◀◀◀ RUN END` 字面常量；内部含 `trimToLastTwoRuns` 私有方法。 | 源码扫描 `GatewayFileLogger.kt`：(1) 含 `fun startRun(` 字面值；(2) 含 `fun endRun(` 字面值；(3) 含 `▶▶▶ RUN START` 字面值；(4) 含 `◀◀◀ RUN END` 字面值；(5) 含 `private fun trimToLastTwoRuns(` 字面值；(6) 含 `lastIndexOf(RUN_START_MARKER)` 字面值（裁剪策略确实按 last START 切）。 | unit-scan | `GatewayFileLoggerRunRetentionWiringTest#TC-GW-DIAG-002-a logger exposes startRun endRun API` 🟡 |
| TC-GW-DIAG-002-b | （调试辅助，无 R-ID） | **`HermesGatewayController.runHermesAgent` 在入口 startRun、在 3 个出口 endRun**。入口标签必须含 `historyChatId`；END 路径分别覆盖 normal / interrupted / early-out 三种 return。 | 源码扫描 `HermesGatewayController.kt`：(1) 含 `GatewayFileLogger.startRun(historyChatId)` 字面值；(2) 含 `GatewayFileLogger.endRun(historyChatId)` 字面值（normal 路径）；(3) 含 `GatewayFileLogger.endRun("$historyChatId (interrupted)")` 字面值；(4) 含 `GatewayFileLogger.endRun("$historyChatId (early-out /new)")` 字面值；(5) `startRun(` 字面值在源码中恰好出现 1 次；(6) `endRun(` 字面值在源码中恰好出现 3 次。 | unit-scan | `GatewayFileLoggerRunRetentionWiringTest#TC-GW-DIAG-002-b controller wires startRun endRun at boundaries` 🟡 |
| TC-GW-DIAG-002-c | （调试辅助，无 R-ID） | **`trimToLastTwoRuns` 策略：扫描末尾 START 标记**。给定包含 0/1/2/3 个 `▶▶▶ RUN START` 标记的文件，行为应是：(0) 不动；(1) 不动（已经只有一次 run）；(≥2) 删除最后一个 START 之前的全部内容。 | 行为测试（用反射或 friend-package 调 private 方法暴露行为）：(0) 空文件 → 内容不变；(1) 文件含 1 个 `▶▶▶ RUN START` → 内容不变；(2) 含 2 个 → 第二个之前全删；(3) 含 3 个 → 仅保留第三个 START 之后的内容。如反射不便，可用源码扫描确认 `lastIndexOf` + `substring` 的实现存在即可。 | unit-scan | `GatewayFileLoggerRunRetentionWiringTest#TC-GW-DIAG-002-c trimToLastTwoRuns retains only the most recent prior run` 🟡 |
| TC-GW-DIAG-002-manual | （调试辅助，无 R-ID） | **端到端手测**：装新 APK，连续触发 ≥ 3 次 gateway run（任意微信/Feishu/Telegram 消息），`cat /sdcard/Download/Hermes/gateway_logs/gateway.log` → 文件中**只有最后两次** `▶▶▶ RUN START` 块，更早的全部被删。 | (1) 文件起始行是 `▶▶▶ RUN START [...]`；(2) 共出现 2 次 `▶▶▶ RUN START`；(3) 共出现 ≥ 1 次 `◀◀◀ RUN END`（最近一次 run 若还没结束可能只有 1 次，正常）；(4) 第三次 run 之前的内容全部消失。 | manual / E2E | `(no unit test; manual verification required)` 🔴 |
